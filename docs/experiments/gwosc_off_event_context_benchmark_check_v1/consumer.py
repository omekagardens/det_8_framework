...
def actual_projection(ri83_body, ri100_body, metadata):
    ...
    need(old.get('schema') == 'ri78-off-event-spectra-v1' and trace.get('schema') == 'ri98-mode-weighted-trace-v1', 'INPUT', 'accepted report schema')
    equal(trace['phase'], 'fixed_saved_application', 'PHASE', 'actual prior phase')
    equal(trace['status'], 'all_gates_passed', 'TRACE', 'accepted prior status')
    segments, traces = [], []
