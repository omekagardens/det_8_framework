...
def verify_ri100_header(value):
    """Verify RI100 header metadata only, after the adapter binds whole bytes.

    A passing header does not establish full-result custody or arithmetic.
    This validator guard does not call or import the primary implementation.
    """
    require(type(value) is dict and all(type(name) is str for name in value),
            'INPUT', 'accepted trace header mapping')
    require(type(value.get('schema')) is str
            and value.get('schema') == 'ri98-mode-weighted-trace-v1'
            and type(value.get('phase')) is str
            and value.get('phase') == 'fixed_saved_application'
            and type(value.get('status')) is str
            and value.get('status') == 'all_declared_checks_passed',
