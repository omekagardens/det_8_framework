"""RI-60 revision of the frozen RI-55 qualifier; no scientific input is opened here.

Adapted from the immutable failed RI-57 qualifier. The exact forward oracle is
reused from its pinned published v1 path; it is not copied or newly authored here.
...
ENGINE_PIN = {"bytes": 30531, "sha256": "ef8169986977c533f549f2ca59b5f72224464db616d8303c451d6a29f22df6af"}
REUSED_ORACLE_PATH = "gwosc_context_operator_v1/oracle.py"
PREDECESSOR_CHECK_PIN = {"bytes": 36039, "sha256": "f68a8f54629f61a2e1e3bf8315c978167074c454efa097e16150d1fb7b3f106e"}
PREDECESSOR_ENGINE_PIN = {"bytes": 24882, "sha256": "3a7b847bd8bec752a48af2c339a5cebd6d923c3b307c2c54910abddbf51703b4"}
...
def load_module(name, path, payload):
    spec = importlib.util.spec_from_file_location("ri60_pinned_" + name, path)
...
def validate_predecessor(predecessor):
    require(type(predecessor) is dict and
            predecessor["schema_version"] == "ri57-context-qualification-v1" and
            predecessor["status"] == "context_qualification_failed" and
            canonical(predecessor["gate_counts"]) == canonical({"total": 196, "passed": 187, "failed": 9}) and
            predecessor["source_identity"] == PREDECESSOR_CHECK_PIN and
            predecessor["engine_identity"] == PREDECESSOR_ENGINE_PIN and
            predecessor["oracle_identity"] == ORACLE_PIN and
            [gate["id"] for gate in predecessor["gates"] if gate["passed"] is not True] ==
            [*("enclosure:" + str(row) for row in ROWS), "structural_constant"],
            "failed RI-57 predecessor identity/status differs")
    return True
...
def admission():
    ...
    payloads["oracle"] = bound_bytes(base / REUSED_ORACLE_PATH, ORACLE_PIN, "reused v1 oracle")
    # All source/report/manifest bytes are bound before parsing or execution.
    predecessor = parse_json(payloads["failed_predecessor"])
    ...
    oracle = load_module("oracle", base / REUSED_ORACLE_PATH, payloads["oracle"])
...
def common_record(admitted):
    return {"schema_version": "ri60-context-qualification-v2", "dimensions": {"N": N, "L": L, "T": T},
        "rows": list(ROWS), "source_identity": identity(Path(__file__).read_bytes()),
        "engine_identity": ENGINE_PIN, "oracle_identity": ORACLE_PIN,
        "reused_oracle_path": REUSED_ORACLE_PATH,
        "failed_predecessor": {"identity": {k: DEPENDENCIES["failed_predecessor"][k] for k in ("bytes", "sha256")},
            "schema_version": "ri57-context-qualification-v1", "status": "context_qualification_failed",
            "gate_counts": {"total": 196, "passed": 187, "failed": 9},
            "interpretation": "Immutable failed v1 attempt; not a qualification receipt for this revision."},
