"""RI-60 driver for the eight RI-55 rows after revised synthetic qualification.

Adapted from the immutable RI-57 driver. The failed predecessor does not admit
this revision, and its exact oracle is explicitly reused at its published path.
...
CHECK_PIN = {"bytes": 38088, "sha256": "a9440a15f31002209ec90519291af510d69813a2c6215b31944beb2eac14fd02"}
...
def load_check(payload, path):
    spec = importlib.util.spec_from_file_location("ri60_pinned_check", path)
...
def source_identity(admitted):
    ...
    return {"process": identity(Path(__file__).read_bytes()), "check": CHECK_PIN,
            "engine": check.ENGINE_PIN, "oracle": check.ORACLE_PIN, "reused_oracle_path": check.REUSED_ORACLE_PATH,
            "published_numeric_dependencies": check.DEPENDENCIES, "published_input_dependencies": DEPENDENCIES}


def base_report(admitted):
    check = admitted["check"]
    return {"schema_version": "ri60-context-sensitivity-v2", "dimensions": {"N": check.N, "L": check.L, "T": check.T},
        ...
        "failed_predecessor": check.common_record(admitted["numeric"])["failed_predecessor"],
