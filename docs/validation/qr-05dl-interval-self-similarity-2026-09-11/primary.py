"""Primary route for QR-05DL: a higher-order manifoldlike invariant -- interval
self-similarity.

Motivation.  A manifoldlike causal set is (statistically) **self-similar**: the interval
I(x,y) of a sprinkle is again a sprinkle of the *same* dimension, so its ordering fraction
should match the global one.  QR-05DK showed the 2-point + link invariants are insufficient
(a geometry-free bipartite order reaches the sprinkle's link fraction).  This gate tests a
genuinely higher-order invariant:

    self-similarity   S = <f(I(x,y))> / f_global        (over intervals of size m_min..m_max)
    interval density  rho_int = P(I(x,y) nonempty) = 1 - link fraction
    dimension match   |d_MM(<f(I)>) - d_MM(f_global)|

FINDING.  Sprinkles are self-similar (S ≈ 1 up to finite-size bias, d_MM of the interval
ordering fraction ≈ the global d).  The QR-05DK counterexamples fail: the degenerate
geometry-free orders (bipartite, layer-cake) have **no intervals** (rho_int = 0, S
undefined); transitive percolation has a large **dimension mismatch** at low ordering
fraction (S up to ~8, |Δd_MM| up to ~2.7).  So interval self-similarity *is* a higher-order
manifoldlike invariant that the counterexamples fail — but manifoldlikeness is
**multi-faceted**: no single invariant (2-point, link, or interval self-similarity) is
sufficient.  Track-B, Status M; no physical claim.
"""
...
def build_report(protocol):
    module = load_module("_qr05dl_t7", MODULE_PATH)
    n = protocol["n"]
    seed = protocol["seed"]
    ...
    sprinkle_self_similar = all(
        r["self_similarity"] is not None and 0.8 <= r["self_similarity"] <= 1.7
        for r in sprinkle_rows)
    degenerate_empty = all(r["interval_density"] < 0.05 or r["intervals_sampled"] == 0
                           for r in degenerate)
    tp_fails_at_low_f = any(
        r["self_similarity"] is not None and r["self_similarity"] > 1.7 for r in tp_rows)

    flags = {
        "sprinkles_are_interval_self_similar": bool(sprinkle_self_similar),
        "degenerate_counterexamples_fail_interval_self_similarity": bool(degenerate_empty),
        "closure_dense_laws_fail_interval_self_similarity_at_low_f": bool(tp_fails_at_low_f),
