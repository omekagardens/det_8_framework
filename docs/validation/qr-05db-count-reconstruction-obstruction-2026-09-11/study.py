"""QR-05DB count-reconstruction obstruction driver.

Runs both routes, refuses to publish on any disagreement, and writes create-only
``source-freeze.json`` / ``results.json``. The full computation is memoised so the
test suite pays for it once.
"""
...
SOURCE_PATHS = ("README.md", "protocol.json", "primary.py", "reference.py",
                "study.py", "test_qr05db.py")
FIXTURE_PATHS = ("../../../det8/models/order_count_geometry.py",)
CAPTURE_SCHEMA = "qr05db-capture-v1"
FREEZE_SCHEMA = "qr05db-source-freeze-v1"
...
def _run():
    ...
    primary = load_module("_qr05db_primary", HERE / "primary.py")
    reference = load_module("_qr05db_reference", HERE / "reference.py")
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="QR-05DB count-reconstruction obstruction")
