"""Create-only exact QR-05D study; --verify replays without rewriting evidence."""
...
def cross_controls(analyses, direct, reference):
    ...
    for past in interiors:
        ...
        require(a == b, "independent interior collision analysis differs")
...
def prior_bridge(direct, reference):
    ...
    for name in ("fork_join", "chain_record", "record_location"):
        ...
        for index in control["histories"]:
            ...
            require(a == b, "independent prior-order diagnostic differs")
...
def run_suite():
    direct = load("qr05d_capture_direct", "geometry.py")
    reference = load("qr05d_capture_reference", "reference_qr05d.py")
    cases = []
