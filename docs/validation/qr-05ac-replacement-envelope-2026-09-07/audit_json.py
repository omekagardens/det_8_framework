"""Independent stdlib JSON-only QR-05AC closed-simplex envelope audit.
...
def audit_capture(path=None):
    ...
    doc = json.loads(data)
    eq(canon(doc), data, "canonical AC envelope bytes")
    fields(doc, "schema_version runtime source_ledger prior_artifacts suite", "AC envelope")
    native_eq(doc["schema_version"], "det8-qr05ac-results-v1", "AC schema")
    ...
    native_eq(sorted(doc["source_ledger"]), sorted(names), "seven exact AC source filenames")
    inherited = []
    ...
    abdoc, aadoc, zdoc, ydoc, xdoc, wdoc = inherited
    native_eq(
    ...
    expected_priors = {**abdoc["prior_artifacts"], AB_PRIOR: AB_ID}
    native_eq(len(expected_priors), 33, "33-prior lineage size")
    native_eq(doc["prior_artifacts"], expected_priors, "exact AC inherited artifact lineage")
    ...
    aasuite, zsuite, ysuite, xsuite, wsuite, suite, acsuite = (
        aadoc["suite"],
        zdoc["suite"],
        ydoc["suite"],
        xdoc["suite"],
    ...
    expected_accases = []
    for accase, case, aacase, zcase, ycase, xcase, wcase in zip(
        acsuite["cases"],
        suite["cases"],
        aasuite["cases"],
        zsuite["cases"],
    ...
    for current in (doc, abdoc, aadoc, zdoc, ydoc, xdoc):
        runtime_metadata(current["runtime"])
    for source, record in frozen.items():
        native_eq(ident(source), record, "after source/prior identity " + str(source))
    native_eq(ident(capture), before, "read-only AC artifact identity")
    analyses = [case["analysis"] for case in expected_accases]
    return {
        "status": "PASS",
        "artifact": before,
