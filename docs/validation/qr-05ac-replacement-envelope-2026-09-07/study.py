"""QR-05AC exact whole-fiber replacement envelopes and read-only replay.

Independent nominal and AB mathematical oracles are statically carried.
New coefficients aggregate original-weight squared distances before extrema.
"""
...
SCHEMA = "det8-qr05ac-results-v1"
BASE_COMMIT = "578543f6480eede54c23d0a74bfe54cca50c8d3e"
SOURCES = (
    "README.md",
    "envelope.py",
    "reference_qr05ac.py",
...
def ab_fixtures():
    ...
    require_same_wire(
        aa["prior_artifacts"],
        {k: v for k, v in priors().items() if k not in (AA_PRIOR, AB_PRIOR)},
...
def fixtures():
    ...
    require_same_wire(
        doc["prior_artifacts"],
        {k: v for k, v in priors().items() if k != AB_PRIOR},
...
def build_witness_evidence(zcase, analysis):
...
def producer_controls(analysis, aacase, zcase, abcase, letters, evidence):
    projected = project_case(aacase, zcase, letters)
    ...
    require_same_wire(
        evidence, build_witness_evidence(zcase, analysis), "complete global witness laws and links"
    )
    scored = score_stress(
        zcase,
        [
...
def check_analysis(analysis, aacase, zcase, abcase, letters, evidence):
    projected = project_case(aacase, zcase, letters)
    require_same_wire(
        analysis, expected_analysis(projected["problem"]), "complete independent AC output"
    )
    return producer_controls(analysis, aacase, zcase, abcase, letters, evidence)


def run_suite():
    data = fixtures()
    letters = alphabet(data["w"])
    primary = load("qr05ac_primary", "envelope.py")
    reference = load("qr05ac_reference", "reference_qr05ac.py")
    cases = []
    for aa, z, ab in zip(data["aa"]["cases"], data["z"]["cases"], data["ab"]["cases"], strict=True):
        case = project_case(aa, z, letters)
        p = case["problem"]
        before = canonical(p)
        left = primary.analyze(p)
        require(canonical(p) == before, "input mutated by primary")
        right = reference.analyze(p)
        require(canonical(p) == before, "input mutated by reference")
        require_same_wire(left, right, "two independent full envelopes")
        evidence = build_witness_evidence(z, left)
        controls = check_analysis(left, aa, z, ab, letters, evidence)
    ...
    invalid.append(p)
    p = copy.deepcopy(cases[0]["problem"])
    p["models"][0]["assumed_index"] = False
    invalid.append(p)
    p = copy.deepcopy(cases[0]["problem"])
    p["models"][0]["full_coefficients"][0][0][0] = [1, 1]
    invalid.append(p)
    p = copy.deepcopy(cases[0]["problem"])
    p["models"][3]["full_coefficients"].pop()
    invalid.append(p)
    p = copy.deepcopy(cases[0]["problem"])
    p["alphabet"][0]["N"][0] = False
    invalid.append(p)
    p = copy.deepcopy(cases[0]["problem"])
    p["models"][0]["full_coefficients"][0][0][1] = 0.5
    invalid.append(p)
    rejected = 0
    for engine in (primary, reference):
        for bad in invalid:
            try:
                ...
                raise ValueError("malformed AC input accepted")
    require(rejected == 16, "all malformed AC controls rejected")
    ...
    return {
        "producer": {"artifact": AB_PRIOR, "bytes": AB_BYTES, "sha256": AB_SHA},
        "alphabet": letters,
        "cases": cases,
