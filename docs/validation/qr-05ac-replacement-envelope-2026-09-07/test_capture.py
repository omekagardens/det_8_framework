...
def runner(tmp_path, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location("qr05ac_lifecycle", path)
...
@pytest.mark.parametrize("verify", [False, True])
def test_artifact_byte_cap(runner, monkeypatch, verify):
    if verify:
        runner.main()
        raw = runner.RESULT.read_bytes()
        monkeypatch.setattr(sys, "argv", ["study.py", "--verify"])
    monkeypatch.setattr(runner, "MAX_CAPTURE_BYTES", 2)
    with pytest.raises(ValueError, match="artifact byte cap"):
        runner.main()
    if verify:
        assert runner.RESULT.read_bytes() == raw
    else:
