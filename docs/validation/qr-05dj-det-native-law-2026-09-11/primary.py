"""Primary route for QR-05DJ: a DET-native (record-κ) law map.

T8 currently *assumes* records do not affect order growth.  This gate tests the opposite:
**record-dependent order feedback**.  Each element carries a record coordinate κ ∈ [0,1];
the relation probability is κ-dependent:

    p_ij = p0 · φ(κ_i, κ_j) ,   φ ∈ { blind: 1 ,  sum: (κ_i+κ_j)/2 ,  similar: 1−|κ_i−κ_j| }

then the order is transitively closed.  ("blind" is the κ-independent control = transitive
percolation.)

TESTS.
  * Manifoldlikeness (O7): at a matched 2-point ordering fraction, the link fraction ℓ vs
    the sprinkle's (the QR-05DI discriminator).
  * Record-legibility (T8 / covariance): the correlation between κ and an order-derived
    statistic (the element's comparability degree) — does the order encode the record?

FINDING (bounded).  Record-dependent growth does **not** close the link mismatch (ℓ ≈ 0.65–0.75
of the sprinkle's), and the "sum" coupling **leaks records into the order** (r ≈ 0.29).  So a
DET-native record-driven law map is **not manifoldlike**, and record feedback trades
manifoldlikeness for record-legibility — O7's emergence is not achieved.  Track-B, Status M.
"""
...
def stats(prec):
    ...
    for (i, j) in comp:
        ...
        if not any(prec[a][k] and prec[k][b] for k in range(n)):
            links += 1
...
def build_report(protocol):
    module = load_module("_qr05dj_t7", MODULE_PATH)
    n = protocol["n"]
    seed = protocol["seed"]
    ...
    for mode in protocol["modes"]:
        ...
        xs = [c["f"] for c in curve]
        order = sorted(range(len(curve)), key=lambda i: xs[i])
        fs = [xs[i] for i in order]
