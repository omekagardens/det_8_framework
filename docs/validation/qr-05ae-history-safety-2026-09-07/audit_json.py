"""Independent stdlib JSON-only QR-05AE history-safety audit.
...
ROOT = HERE.parents[2]
ADDIR = ROOT / "docs/validation/qr-05ad-continuous-retention-2026-09-07"
AD_PRIOR = "qr-05ad-continuous-retention-2026-09-07/results.json"
AD_ID = {
    "bytes": 5003206,
    "sha256": "099c9df18db91545fdd3a08a383e2e4ed96bf5734f6b80a9e492c3bc954c611a",
}
...
# AE independently partitions each shared-alpha profile using exact root order.
AE_STRATUM_CHECKS = (
...
def ae_polynomial(A, B):
    return {"quadratic_coefficient": ad_wire(A, 0, 2), "half_linear_coefficient": ad_wire(B, -2, 2)}


def ae_open_sign(A, B, left, right, root):
    # a is strictly positive in every open stratum. Classify its remaining
    # affine factor by ordered roots, without evaluating any chosen alpha.
    check(0 <= left < right <= 1, "AE exact open domain")
    if A == 0:
    ...
    if 2 * B >= A:
        return -1
    check(root is not None and 0 < root < 1 and A * root == 2 * B, "AE eligible exact root")
    if root <= left:
        return 1
    if root >= right:
        return -1
    raise RuntimeError("AE unpartitioned interior history root")


def expected_ae(problem):
    """Independent complete generic diagnostic, including all zero-group coefficients."""
    native_tree(problem)
    fields(problem, "schema_version family levels histories profiles decisions", "AE problem")
    native_eq(problem["schema_version"], "det8-qr05ae-problem-v1", "AE schema")
    native_eq(problem["family"], "qr05ae_history_safety", "AE family")
    native_eq(problem["levels"], [wire(t) for t in LEVELS], "AE unchanged levels")
    histories = problem["histories"]
    check(type(histories) is list and 1 <= len(histories) <= 128, "AE histories")
    weights = []
    for h, row in enumerate(histories):
        fields(row, "belief_id weight", "AE history")
        native_eq(row["belief_id"], h, "AE native ordered history id")
        weight = rat(row["weight"])
        check(weight > 0, "AE positive history likelihood")
        weights.append(weight)
    eq(sum(weights, F(0)), F(1), "AE original history normalization")
    H = len(weights)
    check(
        type(problem["profiles"]) is list and len(problem["profiles"]) == 16, "AE sixteen profiles"
    )
    coefficient = []
    for index, row in enumerate(problem["profiles"]):
        fields(row, "assumed_index actual_index coefficients", "AE profile")
        native_eq(row["assumed_index"], index // 4, "AE ordered assumed profile")
        native_eq(row["actual_index"], index % 4, "AE ordered actual profile")
        check(
            type(row["coefficients"]) is list and len(row["coefficients"]) == H,
            "AE history coefficient order",
        )
        parsed = []
        for poly in row["coefficients"]:
            fields(poly, "quadratic_coefficient half_linear_coefficient", "AE polynomial")
            parsed.append(
                (
                    score_fraction(poly["quadratic_coefficient"]),
                    signed_fraction(poly["half_linear_coefficient"]),
                )
            )
        coefficient.append(parsed)
    check(
        type(problem["decisions"]) is list and len(problem["decisions"]) == 16,
        "AE sixteen decisions",
    )
    policies = []
    for index, row in enumerate(problem["decisions"]):
        fields(row, "assumed_index bound_index policy", "AE decision")
        native_eq(row["assumed_index"], index // 4, "AE ordered assumed decision")
        native_eq(row["bound_index"], index % 4, "AE ordered bound decision")
        policy = row["policy"]
        check(type(policy) is dict, "AE native policy")
        if policy.get("kind") == "point":
            fields(policy, "kind weight", "AE point policy")
            policies.append(rat(policy["weight"]))
        else:
            native_eq(
                policy,
                {"kind": "interval", "lower": [0, 1], "upper": [1, 1]},
                "AE full interval only",
            )
            policies.append(None)
    occurrences = [(4 * (d // 4) + t, policies[d]) for d in range(16) for t in range(d % 4 + 1)]
    used = sorted({p for p, alpha in occurrences if alpha is None})
    eligible = {p: [h for h, (A, B) in enumerate(coefficient[p]) if 0 < 2 * B < A] for p in used}
    R, I = len(used), sum(len(ids) for ids in eligible.values())
    P = sum(alpha is not None for _, alpha in occurrences)
    Sr = P + sum(2 * len(eligible[p]) + 3 for p, alpha in occurrences if alpha is None)
    Tr = P + sum(len(eligible[p]) + 2 for p, alpha in occurrences if alpha is None)
    Or, Mr, Er = Sr - Tr, H * Sr, (H + 4) * Tr
    Wr = (16 + R) * H + I + 2 * H * Sr + 4 * Tr + Sr + 40
    check(
        I <= 2048 and Sr <= 4096 and Mr <= 524288 and Er <= 600000 and Wr <= 2000000,
        "AE conservative default reservations",
    )
    roots, knots, aggregates, profiles = {}, {}, [], []
    unique_root_count = 0
    for p, rows in enumerate(coefficient):
        aggregate = (
            sum((w * A for w, (A, B) in zip(weights, rows, strict=True)), F(0)),
            sum((w * B for w, (A, B) in zip(weights, rows, strict=True)), F(0)),
        )
        aggregates.append(aggregate)
        records = knot_wire = None
        if p in used:
            roots[p] = {h: 2 * rows[h][1] / rows[h][0] for h in eligible[p]}
            for h, root in roots[p].items():
                eq(root * rows[h][0], 2 * rows[h][1], "AE exact conditional sign root")
                check(0 < root < 1, "AE strictly interior root")
            records = [
                {"history_index": h, "weight": ad_wire(root, 0, 1)} for h, root in roots[p].items()
            ]
            knots[p] = sorted({F(0), F(1), *roots[p].values()})
            unique_root_count += len(knots[p]) - 2
            knot_wire = [wire(alpha) for alpha in knots[p]]
        profiles.append(
            {
                "assumed_index": p // 4,
                "actual_index": p % 4,
                "aggregate_polynomial": ae_polynomial(*aggregate),
    ...
    S = T = O = 0
    decisions = []
    for d, alpha in enumerate(policies):
        s, u = d // 4, d % 4
        worlds = []
        for t in range(u + 1):
            p = 4 * s + t
            if alpha is not None:
                regions = [("point", alpha, alpha)]
            else:
                values = knots[p]
                regions = []
                for i, value in enumerate(values):
                    ...
                    if i + 1 < len(values):
                        regions.append(("open_interval", value, values[i + 1]))
                eq(len(regions), 2 * len(values) - 1, "AE complete knot/open domain")
                eq(regions[0], ("point", F(0), F(0)), "AE domain includes zero")
                eq(regions[-1], ("point", F(1), F(1)), "AE domain includes one")
            strata, harmed, benefited = [], set(), set()
            for kind, left, right in regions:
                point = kind == "point"
                S += 1
                T += int(point)
                O += int(not point)
                if point:
                    history_values = [A * left * left - 2 * B * left for A, B in coefficient[p]]
                    signs = [(v > 0) - (v < 0) for v in history_values]
                    for value in history_values:
                        ad_wire(value, -4, 6)
                else:
                    history_values = None
                    signs = [
                        ae_open_sign(A, B, left, right, roots[p].get(h))
                        for h, (A, B) in enumerate(coefficient[p])
                    ]
                groups, sums = {}, {}
                for name, sign in (("positive", 1), ("negative", -1), ("zero", 0)):
                    ids = [h for h in range(H) if signs[h] == sign]
                    mass = sum((weights[h] for h in ids), F(0))
                    A = sum((weights[h] * coefficient[p][h][0] for h in ids), F(0))
                    B = sum((weights[h] * coefficient[p][h][1] for h in ids), F(0))
                    eq(mass > 0, bool(ids), "AE strict original-weight group mass")
                    if not ids:
                        eq((mass, A, B), (F(0), F(0), F(0)), "AE empty group polynomial")
                    sums[name] = (mass, A, B)
                    groups[name] = {
                        "history_indices": ids,
                        "likelihood_mass": wire(mass),
                        "polynomial": ae_polynomial(A, B),
                    }
                native_eq(
                    sorted(h for g in groups.values() for h in g["history_indices"]),
                    list(range(H)),
                    "AE disjoint complete sign memberships",
                )
                eq(sum((v[0] for v in sums.values()), F(0)), F(1), "AE three original masses")
                eq(
                    (
                        sum((v[1] for v in sums.values()), F(0)),
                        sum((v[2] for v in sums.values()), F(0)),
                    ),
                    aggregates[p],
                    "AE full three-group polynomial identity",
                )
                if point:
                    evaluations = {}
                    for name, (_, A, B) in sums.items():
                        value = A * left * left - 2 * B * left
                        direct = sum(
                            (
                                weights[h] * history_values[h]
                                for h in groups[name]["history_indices"]
                            ),
                            F(0),
                        )
                        eq(value, direct, "AE group polynomial and conditional risk sum")
                        evaluations[name] = value
                    A, B = aggregates[p]
                    aggregate = A * left * left - 2 * B * left
                    eq(
                        aggregate,
                        sum((w * v for w, v in zip(weights, history_values, strict=True)), F(0)),
                        "AE aggregate direct weighted conditional risks",
                    )
                    eq(
                        aggregate,
                        sum(evaluations.values(), F(0)),
                        "AE all three point contributions",
                    )
                    check(
                        evaluations["positive"] >= 0
                        and evaluations["negative"] <= 0
                        and evaluations["zero"] == 0,
                        "AE conditional sign contributions",
                    )
                    eq(
                        aggregate,
                        evaluations["positive"] + evaluations["negative"],
                        "AE evaluated P+N with zero value",
                    )
                    point_values = {
                        "history_excesses": [ad_wire(v, -4, 6) for v in history_values],
                        "positive_contribution": ad_wire(evaluations["positive"], 0, 6),
                        "negative_contribution": ad_wire(evaluations["negative"], -4, 0),
                        "zero_contribution": ad_wire(evaluations["zero"], 0, 0),
                        "aggregate_excess": ad_wire(aggregate, -4, 6),
                        "benefit_magnitude": ad_wire(-evaluations["negative"], 0, 4),
                    }
                    region = {"kind": "point", "weight": wire(left)}
                else:
                    eq(sums["zero"][1:], (F(0), F(0)), "AE open zero group identically zero")
                    point_values = None
                    region = {"kind": "open_interval", "lower": wire(left), "upper": wire(right)}
                harmed.update(groups["positive"]["history_indices"])
                benefited.update(groups["negative"]["history_indices"])
                strata.append(
                    {
                        "region": region,
                        "groups": groups,
                        "point_values": point_values,
                        "checks": dict.fromkeys(AE_STRATUM_CHECKS, True),
                    }
                )
            worlds.append(
                {
                    ...
                    "profile_index": p,
                    "strata": strata,
                    "potentially_harmed_history_indices": sorted(harmed),
                    "potentially_benefited_history_indices": sorted(benefited),
                    "all_policy_weights_history_safe": not harmed,
                    "checks": dict.fromkeys(AE_WORLD_CHECKS, True),
                }
            )
        native_eq(
            [w["actual_index"] for w in worlds],
            list(range(u + 1)),
            "AE complete original world set",
        )
        decisions.append(
            {
                "assumed_index": s,
                "bound_index": u,
                "policy": problem["decisions"][d]["policy"],
                "worlds": worlds,
                "harm_possible_world_indices": [
                    w["actual_index"] for w in worlds if w["potentially_harmed_history_indices"]
                ],
                "checks": dict.fromkeys(AE_DECISION_CHECKS, True),
            }
        )
    counts = {
        "histories": H,
        ...
        "distinct_interior_roots": unique_root_count,
        "strata": S,
        "point_strata": T,
        "open_strata": O,
        ...
        "total_work_terms": (16 + R) * H + I + 2 * H * S + 4 * T + S + 40,
    }
    check(
        S <= Sr and T <= Tr and O <= Or and counts["total_work_terms"] <= Wr,
        "AE executed deduplicated work bounded by reservation",
    )
    for key, value in counts.items():
        COUNTS["AE_declared_" + key] += value
    result = {
        "input_sha256": digest(problem),
    ...
    native_tree(result)
    check(len(canon(result)) <= 33554432, "AE full generic wire cap")
    return json.loads(canon(result))


def ae_literal_polynomial(coarse, detailed, outcome):
    quadratic, half_linear = F(0), F(0)
    for coordinate in set(coarse) | set(detailed) | {outcome}:
        difference = detailed.get(coordinate, F(0)) - coarse.get(coordinate, F(0))
        residual = int(coordinate == outcome) - coarse.get(coordinate, F(0))
        quadratic += difference * difference
        half_linear += residual * difference
        COUNTS["AE_literal_polynomial_coordinates"] += 1
    return quadratic, half_linear


def ae_literal_loss(forecast, outcome):
    value = F(0)
    for coordinate in set(forecast) | {outcome}:
        value += (int(coordinate == outcome) - forecast.get(coordinate, F(0))) ** 2
        COUNTS["AE_literal_point_loss_coordinates"] += 1
    return value


def ae_conditional_profiles(contexts, baseline):
    histories = [{"belief_id": ctx["belief_id"], "weight": ctx["weight_wire"]} for ctx in contexts]
    rows, joint_cache = [], {}
    for s in range(4):
        links = baseline["witness_certificate_map"][4 * s : 4 * s + 4]
        eq(
            len({row["witness_id"] for row in links}),
            1,
            "AE unchanged common mechanism across bounds",
        )
        wid = links[0]["witness_id"]
        witness = baseline["witness_laws"][wid]
        for t in range(4):
            conditional = []
            for h, (ctx, belief) in enumerate(zip(contexts, witness["beliefs"], strict=True)):
                native_eq(belief["belief_id"], h, "AE original witness history order")
                native_eq(belief["weight"], ctx["weight_wire"], "AE unchanged original likelihood")
                native_eq(
                    belief["channels"][t]["level"], wire(LEVELS[t]), "AE unchanged actual level"
                )
                joint = ab_joint(belief["channels"][t]["cells"])
                eq(
                    ab_prefix_joint(joint),
                    ab_prefix_joint(ctx["clean"]),
                    "AE conditional N/future marginal",
                )
                joint_cache[s, t, h] = joint
                A, B = F(0), F(0)
                for (value, q), mass in joint.items():
                    coarse = ctx["coarse"][value[:5]]
                    detailed = ctx["frozen"][s][value][2]
                    term_A, term_B = ae_literal_polynomial(coarse, detailed, q)
                    # These are conditional on H. The original history weight
                    # is deliberately absent until case/group aggregation.
                    A += mass * term_A
                    B += mass * term_B
                    COUNTS["AE_conditional_source_atom_polynomials"] += 1
                conditional.append(ae_polynomial(A, B))
                COUNTS["AE_conditional_history_profiles"] += 1
            rows.append({"assumed_index": s, "actual_index": t, "coefficients": conditional})
    policies = [
        {
            "assumed_index": row["assumed_index"],
            "bound_index": row["bound_index"],
            "policy": row["optimizer"],
        }
        for row in baseline["analysis"]["decisions"]
    ]
    problem = {
    ...
    return problem, joint_cache


def ae_literal_history_boundaries(contexts, baseline, problem, analysis, joint_cache):
    boundaries = sorted(
        {
            (decision["assumed_index"], world["actual_index"], rat(stratum["region"]["weight"]))
            for decision in analysis["decisions"]
            for world in decision["worlds"]
            for stratum in world["strata"]
            if stratum["region"]["kind"] == "point"
        }
    )
    cache = {}
    for s, t, alpha in boundaries:
        values = []
        for h, ctx in enumerate(contexts):
            value = F(0)
            for (report, q), mass in joint_cache[s, t, h].items():
                coarse, detailed = ctx["coarse"][report[:5]], ctx["frozen"][s][report][2]
                forecast = {
                    k: (1 - alpha) * coarse.get(k, F(0)) + alpha * detailed.get(k, F(0))
                    for k in set(coarse) | set(detailed)
                }
                eq(sum(forecast.values(), F(0)), F(1), "AE exact shared-alpha forecast law")
                value += mass * (ae_literal_loss(forecast, q) - ae_literal_loss(coarse, q))
                COUNTS["AE_literal_boundary_source_atoms"] += 1
            ad_wire(value, -2, 2)
            values.append(value)
            COUNTS["AE_literal_conditional_boundary_scores"] += 1
        cache[s, t, alpha] = values
    for decision in analysis["decisions"]:
        s, u = decision["assumed_index"], decision["bound_index"]
        old_decision = baseline["analysis"]["decisions"][4 * s + u]
        native_eq(decision["policy"], old_decision["optimizer"], "AE complete AD policy preserved")
        for world in decision["worlds"]:
            t = world["actual_index"]
            for stratum in world["strata"]:
                if stratum["region"]["kind"] != "point":
                    check(
                        stratum["point_values"] is None, "AE no interval midpoint or representative"
                    )
                    continue
                alpha = rat(stratum["region"]["weight"])
                raw_values = cache[s, t, alpha]
                native_eq(
                    [signed_wire(v) for v in raw_values],
                    stratum["point_values"]["history_excesses"],
                    "AE every conditional history boundary risk literally rescored",
                )
                weighted = sum(
                    (ctx["weight"] * v for ctx, v in zip(contexts, raw_values, strict=True)), F(0)
                )
                eq(
                    weighted,
                    F(*stratum["point_values"]["aggregate_excess"]),
                    "AE literal boundary risks recover complete case average",
                )
                for name, field in (
                    ...
                    contribution = sum(
                        (
                            contexts[h]["weight"] * raw_values[h]
                            for h in stratum["groups"][name]["history_indices"]
                        ),
                        F(0),
                    )
                    eq(
                        contribution,
                        F(*stratum["point_values"][field]),
                        "AE literal signed group cancellation remains visible",
                    )
                COUNTS["AE_mapped_boundary_strata_checked"] += 1
    COUNTS["AE_unique_shared_alpha_boundaries"] += len(boundaries)


def ae_producer_controls(contexts, baseline, problem, analysis):
    native_eq(
        problem["histories"],
        [{"belief_id": c["belief_id"], "weight": c["weight_wire"]} for c in contexts],
        "AE entire original history weight table",
    )
    for s in range(4):
        all_worlds = baseline["world_certificates"][4 * s + 3]["polynomials"]
        for t in range(4):
            profile = analysis["profiles"][4 * s + t]
            expected = {
                k: all_worlds[t][k] for k in ("quadratic_coefficient", "half_linear_coefficient")
            }
            native_eq(
                profile["aggregate_polynomial"],
                expected,
                "AE weighted conditional coefficients reproduce all AD world polynomials",
            )
            COUNTS["AE_AD_aggregate_polynomials_checked"] += 1
    for index, decision in enumerate(analysis["decisions"]):
        original = baseline["analysis"]["decisions"][index]
        world_evidence = baseline["world_certificates"][index]
        native_eq(
            decision["policy"], original["optimizer"], "AE unchanged point/whole interval optimizer"
        )
        if decision["policy"]["kind"] == "point":
            aggregates = [
                world["strata"][0]["point_values"]["aggregate_excess"]
                for world in decision["worlds"]
            ]
            native_eq(
                aggregates,
                world_evidence["point_world_excesses"],
                "AE original complete AD point-world vector",
            )
            eq(
                max(F(*v) for v in aggregates),
                F(*original["minimum"]),
                "AE unchanged AD point bound minimum",
            )
        else:
            check(
                world_evidence["whole_interval_minimax"]
                and world_evidence["point_world_excesses"] is None
                and world_evidence["point_worst_world_indices"] is None,
                "AE preserves AD parametric rather than point evidence",
            )
            for world in decision["worlds"]:
                poly = analysis["profiles"][world["profile_index"]]["aggregate_polynomial"]
                A, B = (
                    score_fraction(poly["quadratic_coefficient"]),
                    signed_fraction(poly["half_linear_coefficient"]),
                )
                check(
                    A >= 0 and A - 2 * B <= 0, "AE inherited whole-interval lower-world inequality"
                )
            final_poly = analysis["profiles"][decision["worlds"][-1]["profile_index"]][
                "aggregate_polynomial"
            ]
            native_eq(final_poly, ae_polynomial(F(0), F(0)), "AE inherited zero upper polynomial")
    return dict.fromkeys(
        (
...
def ae_projection(contexts, baseline):
    problem, joints = ae_conditional_profiles(contexts, baseline)
    analysis = expected_ae(problem)
    ae_literal_history_boundaries(contexts, baseline, problem, analysis, joints)
    controls = ae_producer_controls(contexts, baseline, problem, analysis)
    retained_baseline = json.loads(canon(baseline))
    native_eq(retained_baseline, baseline, "AE entire AD case retained unchanged")
    return {
        "problem": problem,
        "analysis": analysis,
...
def audit_capture(path=None):
    ...
    doc = json.loads(data)
    eq(canon(doc), data, "canonical AE envelope bytes")
    fields(doc, "schema_version runtime source_ledger prior_artifacts suite", "AE envelope")
    native_eq(doc["schema_version"], "det8-qr05ae-results-v1", "AE schema")
    ...
    native_eq(sorted(doc["source_ledger"]), sorted(names), "seven exact AE source filenames")
    inherited = []
    ...
    addoc, acdoc, abdoc, aadoc, zdoc, ydoc, xdoc, wdoc = inherited
    native_eq(
    ...
    native_eq(
        addoc["prior_artifacts"],
        {**acdoc["prior_artifacts"], AC_PRIOR: AC_ID},
        "AD inherited artifact lineage",
    )
    expected_priors = {**addoc["prior_artifacts"], AD_PRIOR: AD_ID}
    native_eq(len(expected_priors), 35, "35-prior lineage size")
    native_eq(doc["prior_artifacts"], expected_priors, "exact AE inherited artifact lineage")
    for prior, primary, reference, tests, gate in (
        (addoc, "retention.py", "reference_qr05ad.py", "test_qr05ad.py", "AD"),
    ...
    aasuite, zsuite, ysuite, xsuite, wsuite, suite, acsuite, adsuite, aesuite = (
        aadoc["suite"],
        zdoc["suite"],
        ydoc["suite"],
        xdoc["suite"],
    ...
    expected_accases, expected_adcases, expected_aecases = [], [], []
    for aecase, adcase, accase, case, aacase, zcase, ycase, xcase, wcase in zip(
        aesuite["cases"],
        adsuite["cases"],
        acsuite["cases"],
        suite["cases"],
        ...
        aecase_expected = {"case_id": aecase["case_id"], **ae_projection(contexts, adcase_expected)}
        native_eq(
            aecase_expected,
            aecase,
            "entire AE independent conditional-history diagnostic and AD baseline",
        )
        expected_aecases.append(aecase_expected)
    ...
    expected_aesuite = {
        "producer": {"artifact": AD_PRIOR, **AD_ID},
        "cases": expected_aecases,
        "independent_route_equal": True,
        "public_controls": {
            "analyze_calls": 12,
            "invalid_analyze_calls_rejected": 16,
        ...
        "totals": suite_totals(expected_aecases, invalid_calls=16),
    }
    native_eq(aesuite, expected_aesuite, "complete AE native canonical reporting closure")
    for current in (doc, addoc, acdoc, abdoc, aadoc, zdoc, ydoc, xdoc):
        runtime_metadata(current["runtime"])
    for source, record in frozen.items():
        native_eq(ident(source), record, "after source/prior identity " + str(source))
    native_eq(ident(capture), before, "read-only AE artifact identity")
    analyses = [case["analysis"] for case in expected_aecases]
    return {
        "status": "PASS",
        "artifact": before,
        "analysis_list": {"bytes": len(canon(analyses)), "sha256": digest(analyses)},
        "authenticated_AD_analysis_list": {
            "bytes": len(canon([case["analysis"] for case in expected_adcases])),
            "sha256": digest([case["analysis"] for case in expected_adcases]),
        },
