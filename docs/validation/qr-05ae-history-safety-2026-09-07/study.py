"""QR-05AE fixed-policy conditional-history safety and read-only replay.

Independent nominal/AB/AC/AD mathematical oracles are statically carried.
Conditional-history coefficients exclude original history weights until aggregation.
"""
...
SCHEMA = "det8-qr05ae-results-v1"
BASE_COMMIT = "070f4795ddf48513839a1e85f3121681c943c645"
SOURCES = (
    "README.md",
    "safety.py",
    "reference_qr05ae.py",
...
def ab_fixtures():
    ...
    require_same_wire(
        aa["prior_artifacts"],
        {k: v for k, v in priors().items() if k not in (AA_PRIOR, AB_PRIOR, AC_PRIOR, AD_PRIOR)},
...
def ac_fixtures():
    ...
    require_same_wire(
        doc["prior_artifacts"],
        {k: v for k, v in priors().items() if k not in (AB_PRIOR, AC_PRIOR, AD_PRIOR)},
...
def ad_fixtures():
    ...
    require_same_wire(
        doc["prior_artifacts"],
        {k: v for k, v in priors().items() if k not in (AC_PRIOR, AD_PRIOR)},
...
def ad_clean_coefficients(zcase, accase):
...
def ad_project_case(accase, zcase, letters):
    ...
    coefficients = ad_clean_coefficients(zcase, accase)
    decisions = []
...
def ad_expected_analysis(problem):
...
def ad_build_evidence(zcase, accase, analysis, coefficients, letters):
    policies = policy_maps(zcase)
...
def ad_literal_polynomials(zcase, witness, assumed):
...
def ad_literal_family_scores(family, witness):
...
def ad_producer_controls(analysis, accase, zcase, aacase, abcase, letters, evidence):
    ...
    projected = ad_project_case(accase, zcase, letters)
    require_same_wire(
        analysis, ad_expected_analysis(projected["problem"]), "complete independent AD output"
    )
    expected = ad_build_evidence(
        zcase, accase, analysis, projected["producer_coefficients"], letters
    )
    ...
    for s, coeff in enumerate(projected["producer_coefficients"]):
        ...
        literal = ad_literal_polynomials(zcase, evidence["witness_laws"][wid], s)
        d, c, full = (
    ...
    for family in evidence["forecast_families"]:
        if family["optimizer"]["kind"] == "point":
            ...
            wid = evidence["witness_certificate_map"][4 * s]["witness_id"]
            values = ad_literal_family_scores(family, evidence["witness_laws"][wid])
...
def ad_check_analysis(analysis, accase, zcase, aacase, abcase, letters, evidence):
    return ad_producer_controls(analysis, accase, zcase, aacase, abcase, letters, evidence)
...
def fixtures():
    result = ad_fixtures()
    raw = plain_bytes(HERE.parent / AD_PRIOR)
    doc = prior_json(AD_PRIOR)
    require(len(raw) == AD_BYTES and canonical(doc) == raw, "canonical pinned AD")
    require_same_wire(
        doc["prior_artifacts"],
        {k: v for k, v in priors().items() if k != AD_PRIOR},
        "AD complete prior lineage",
    )
    result["ad"] = doc["suite"]
    require_same_wire(
        [c["case_id"] for c in result["ad"]["cases"]],
        [c["case_id"] for c in result["aa"]["cases"]],
        "AD/AA case order",
    )
    return result
...
def polynomial(a, b):
    require(0 <= a <= 2 and -2 <= b <= 2, "weighted convex polynomial range")
    return {"quadratic_coefficient": wire(a), "half_linear_coefficient": wire(b)}
...
def conditional_profiles(adcase, zcase):
    """Original conditional-report vector route; no history weight inside A_H/B_H."""
    sources = zcase["problem"]["experiment"]["beliefs"]
    policies = policy_maps(zcase)
    histories = [
        {"belief_id": b["belief_id"], "weight": copy.deepcopy(b["weight"])} for b in sources
    ]
    require([b["belief_id"] for b in histories] == list(range(len(histories))), "history ID order")
    require(
        sum((fraction(b["weight"]) for b in histories), F(0)) == 1
        and all(fraction(b["weight"]) > 0 for b in histories),
        "positive original weights",
    )
    profiles = []
    for s in range(4):
        ...
        require(
            all(link["witness_id"] == wid for link in links), "one common witness across bounds"
        )
        witness = adcase["witness_laws"][wid]
        require(len(witness["beliefs"]) == len(histories), "complete witness histories")
        for t in range(4):
            rows = []
            for source, actual, models in zip(sources, witness["beliefs"], policies, strict=True):
                require_same_wire(actual["belief_id"], source["belief_id"], "same witness history")
                require_same_wire(
                    actual["weight"], source["weight"], "same original history weight"
                )
                channel = actual["channels"][t]
                require_same_wire(channel["level"], LEVELS[t], "same actual world")
                a, b = F(0), F(0)
                require(
                    sum((fraction(c["probability"]) for c in channel["cells"]), F(0)) == 1,
                    "conditional history channel normalization",
                )
                for cell in channel["cells"]:
                    mass = fraction(cell["probability"])
                    truth = prediction(cell["prediction"])
                    g, f, _ = models[s][tuple(cell["value"])]
                    coords = truth.keys() | g.keys() | f.keys()
                    for q in coords:
                        delta = f.get(q, F(0)) - g.get(q, F(0))
                        a += mass * delta * delta
                        b += mass * (truth.get(q, F(0)) - g.get(q, F(0))) * delta
                rows.append(polynomial(a, b))
            profiles.append({"assumed_index": s, "actual_index": t, "coefficients": rows})
    return histories, profiles


def project_case(adcase, zcase):
    require_same_wire(adcase["case_id"], zcase["case_id"], "same AE source case")
    histories, profiles = conditional_profiles(adcase, zcase)
    decisions = [
        {
            "assumed_index": d["assumed_index"],
            "bound_index": d["bound_index"],
            "policy": copy.deepcopy(d["optimizer"]),
        }
        for d in adcase["analysis"]["decisions"]
    ]
    return {
...
def stratum_oracle(rows, weights, aggregate, region):
    """Endpoint factor signs on open regions, not a sampled representative."""
    is_point = region["kind"] == "point"
    values = None
    if is_point:
        x = fraction(region["weight"])
        values = [a * x * x - 2 * b * x for a, b in rows]
        signs = [sign(v) for v in values]
    else:
        lo, hi = fraction(region["lower"]), fraction(region["upper"])
        require(0 <= lo < hi <= 1, "ordered open region")
        signs = []
        for a, b in rows:
            left, right = a * lo - 2 * b, a * hi - 2 * b
            if a == b == 0:
                signs.append(0)
                ...
                raise ValueError("open region crosses a conditional-history root")
    groups, sums, masses = {}, {}, {}
    for name, target in (("positive", 1), ("negative", -1), ("zero", 0)):
        ids = [i for i, v in enumerate(signs) if v == target]
        mass = sum((weights[i] for i in ids), F(0))
        a = sum((weights[i] * rows[i][0] for i in ids), F(0))
        b = sum((weights[i] * rows[i][1] for i in ids), F(0))
        require((mass > 0) == bool(ids), "positive history weights and group masses")
        groups[name] = {
            "history_indices": ids,
        ...
        masses[name] = mass
    require(
        sorted(i for g in groups.values() for i in g["history_indices"]) == list(range(len(rows))),
        "complete disjoint sign partition",
    )
    require(sum(masses.values(), F(0)) == 1, "original-weight group mass closure")
    require(
        tuple(sum((pair[j] for pair in sums.values()), F(0)) for j in range(2)) == aggregate,
        "all three weighted group polynomials close coefficientwise",
    )
    point = None
    if is_point:
        group_values = {name: a * x * x - 2 * b * x for name, (a, b) in sums.items()}
        for name, group in groups.items():
            require(
                group_values[name]
                == sum((weights[i] * values[i] for i in group["history_indices"]), F(0)),
                "group polynomial equals direct weighted conditional risk",
            )
        a, b = aggregate
        total = a * x * x - 2 * b * x
        require(
            total == sum((w * v for w, v in zip(weights, values, strict=True)), F(0)),
            "complete weighted point risk",
        )
        require(
            group_values["positive"] >= 0
            and group_values["negative"] <= 0
            and group_values["zero"] == 0
            and total == sum(group_values.values(), F(0)),
            "signed point contribution decomposition",
        )
        point = {
            "history_excesses": [scalar(v, F(-4), F(6)) for v in values],
            "positive_contribution": scalar(group_values["positive"], F(0), F(6)),
            "negative_contribution": scalar(group_values["negative"], F(-4), F(0)),
            "zero_contribution": [0, 1],
            "aggregate_excess": scalar(total, F(-4), F(6)),
            "benefit_magnitude": scalar(-group_values["negative"], F(0), F(4)),
        }
    else:
        require(sums["zero"] == (F(0), F(0)), "open zero group is identically zero")
    return {
...
def expected_analysis(problem):
    """Independent direct coefficient/sign oracle; no optimizer or policy fitting."""
    histories = problem["histories"]
    h = len(histories)
    weights = [fraction(b["weight"]) for b in histories]
    inputs = [[coefficients(row) for row in p["coefficients"]] for p in problem["profiles"]]
    used = {
        4 * d["assumed_index"] + t
        for d in problem["decisions"]
        if d["policy"]["kind"] == "interval"
        for t in range(d["bound_index"] + 1)
    }
    eligible = {p: [i for i, (a, b) in enumerate(inputs[p]) if 0 < 2 * b < a] for p in used}
    pcount = sum(
        d["bound_index"] + 1 for d in problem["decisions"] if d["policy"]["kind"] == "point"
    )
    sr = pcount + sum(
        2 * len(eligible[4 * d["assumed_index"] + t]) + 3
        for d in problem["decisions"]
        if d["policy"]["kind"] == "interval"
        for t in range(d["bound_index"] + 1)
    )
    tr = pcount + sum(
        len(eligible[4 * d["assumed_index"] + t]) + 2
        for d in problem["decisions"]
        if d["policy"]["kind"] == "interval"
        for t in range(d["bound_index"] + 1)
    )
    roots_count = sum(map(len, eligible.values()))
    reserved_work = (16 + len(used)) * h + roots_count + 2 * h * sr + 4 * tr + sr + 40
    require(
        h <= 128
        and roots_count <= 2048
        and sr <= 4096
        and h * sr <= 524288
        and (h + 4) * tr <= 600000
        and reserved_work <= 2000000,
        "conservative AE reservations",
    )
    profiles, aggregates = [], []
    for p, rows in enumerate(inputs):
        aggregate = tuple(
            sum((w * pair[j] for w, pair in zip(weights, rows, strict=True)), F(0))
            for j in range(2)
        )
        ...
        if p in used:
            records = [
                {"history_index": i, "weight": scalar(2 * rows[i][1] / rows[i][0], F(0), F(1))}
                for i in eligible[p]
            ]
            knots = [
                wire(v) for v in sorted({F(0), F(1)} | {fraction(r["weight"]) for r in records})
            ]
        profiles.append(
            {
                "assumed_index": p // 4,
                "actual_index": p % 4,
                "aggregate_polynomial": polynomial(*aggregate),
                "interval_root_records": records,
                "interval_knots": knots,
            }
        )
    decisions, point_strata, open_strata = [], 0, 0
    for d in problem["decisions"]:
        s, u, policy = d["assumed_index"], d["bound_index"], d["policy"]
        worlds = []
        for t in range(u + 1):
            p = 4 * s + t
            if policy["kind"] == "point":
                regions = [{"kind": "point", "weight": copy.deepcopy(policy["weight"])}]
            else:
                knots = profiles[p]["interval_knots"]
                regions = []
                for j, knot in enumerate(knots):
                    if j:
                        regions.append(
                            {
                                "kind": "open_interval",
                                "lower": copy.deepcopy(knots[j - 1]),
                                "upper": copy.deepcopy(knot),
                            }
                        )
                    regions.append({"kind": "point", "weight": copy.deepcopy(knot)})
            strata = [
                stratum_oracle(inputs[p], weights, aggregates[p], region) for region in regions
            ]
            point_strata += sum(row["region"]["kind"] == "point" for row in strata)
            open_strata += sum(row["region"]["kind"] == "open_interval" for row in strata)
            positive = sorted(
                {i for row in strata for i in row["groups"]["positive"]["history_indices"]}
            )
            negative = sorted(
                {i for row in strata for i in row["groups"]["negative"]["history_indices"]}
            )
            worlds.append(
                {
                    "actual_index": t,
                    "profile_index": p,
                    "strata": strata,
                    "potentially_harmed_history_indices": positive,
                    "potentially_benefited_history_indices": negative,
    ...
    strata_count = point_strata + open_strata
    work = (
        (16 + len(used)) * h
        + roots_count
        + 2 * h * strata_count
    ...
    require(
        strata_count <= sr and point_strata <= tr and work <= reserved_work,
        "deduplicated execution below conservative reservation",
    )
    counts = {
        "histories": h,
        "profiles": 16,
        ...
        "point_world_certificates": pcount,
        "interval_world_certificates": 40 - pcount,
        "interval_profiles": len(used),
        "interior_root_candidates": roots_count,
        "distinct_interior_roots": sum(len(profiles[p]["interval_knots"]) - 2 for p in used),
        "strata": strata_count,
        "point_strata": point_strata,
        "open_strata": open_strata,
        "history_memberships": h * strata_count,
        "reserved_strata": sr,
        ...
        "root_classification_terms": len(used) * h,
        "root_divisions": roots_count,
        "history_point_evaluations": h * point_strata,
        "group_point_evaluations": 3 * point_strata,
        "aggregate_point_evaluations": point_strata,
        "open_sign_classifications": h * open_strata,
        "group_accumulation_terms": h * strata_count,
        "stratum_certifications": strata_count,
        "world_certifications": 40,
        "total_work_terms": work,
    }
    return {
        "input_sha256": digest(canonical(problem)),
        "levels": copy.deepcopy(LEVELS),
...
def verify_ad_case(adcase, zcase, accase, aacase, abcase, letters):
    projected = ad_project_case(accase, zcase, letters)
    expected = ad_expected_analysis(projected["problem"])
    evidence = ad_build_evidence(
        zcase, accase, expected, projected["producer_coefficients"], letters
    )
    controls = ad_check_analysis(
        adcase["analysis"], accase, zcase, aacase, abcase, letters, {k: adcase[k] for k in evidence}
    )
    require_same_wire(
        adcase,
        {**projected, "analysis": expected, **evidence, "producer_controls": controls},
        "complete independent AD baseline preserved",
    )


def literal_history_scores(adcase, zcase, s, t, weight):
    """Literal Brier differences, conditional on H; no original w_H factor."""
    wid = adcase["witness_certificate_map"][4 * s]["witness_id"]
    witness = adcase["witness_laws"][wid]
    values = []
    for belief, models in zip(witness["beliefs"], policy_maps(zcase), strict=True):
        total = F(0)
        for cell in belief["channels"][t]["cells"]:
            truth = prediction(cell["prediction"])
            g, f, _ = models[s][tuple(cell["value"])]
            h = {
                q: (1 - weight) * g.get(q, F(0)) + weight * f.get(q, F(0))
                for q in g.keys() | f.keys()
            }
            require(sum(h.values(), F(0)) == 1, "complete fixed-segment boundary forecast")
            total += fraction(cell["probability"]) * (
                literal_loss(truth, h) - literal_loss(truth, g)
            )
        require(-2 <= total <= 2, "conditional literal Brier-excess range")
        values.append(total)
    return values


def check_analysis(analysis, adcase, zcase, accase, aacase, abcase, letters):
    verify_ad_case(adcase, zcase, accase, aacase, abcase, letters)
    projected = project_case(adcase, zcase)
    expected = expected_analysis(projected["problem"])
    require_same_wire(analysis, expected, "complete independent AE diagnostic")
    for p, profile in enumerate(analysis["profiles"]):
        s, t = divmod(p, 4)
        old = adcase["world_certificates"][4 * s + 3]["polynomials"][t]
        require_same_wire(
            profile["aggregate_polynomial"],
            {k: old[k] for k in ("quadratic_coefficient", "half_linear_coefficient")},
            "all history-weighted coefficients recover AD world polynomial",
        )
    literal = {}
    for decision in analysis["decisions"]:
        s, u = decision["assumed_index"], decision["bound_index"]
        previous = adcase["world_certificates"][4 * s + u]
        require_same_wire(decision["policy"], previous["optimizer"], "unchanged global AD policy")
        values = []
        for world in decision["worlds"]:
            t = world["actual_index"]
            for stratum in world["strata"]:
                if stratum["region"]["kind"] != "point":
                    require(stratum["point_values"] is None, "interval interiors remain parametric")
                    continue
                weight = fraction(stratum["region"]["weight"])
                key = (s, t, weight)
                if key not in literal:
                    literal[key] = literal_history_scores(adcase, zcase, s, t, weight)
                require_same_wire(
                    stratum["point_values"]["history_excesses"],
                    [wire(v) for v in literal[key]],
                    "every conditional history literal score",
                )
                original_weights = [
                    fraction(b["weight"]) for b in projected["problem"]["histories"]
                ]
                aggregate = sum(
                    (w * v for w, v in zip(original_weights, literal[key], strict=True)), F(0)
                )
                require_same_wire(
                    stratum["point_values"]["aggregate_excess"],
                    wire(aggregate),
                    "literal original-weight case aggregation",
                )
            if decision["policy"]["kind"] == "point":
                require(len(world["strata"]) == 1, "one unchanged point policy")
                values.append(world["strata"][0]["point_values"]["aggregate_excess"])
            else:
                poly = analysis["profiles"][world["profile_index"]]["aggregate_polynomial"]
                a, b = coefficients(poly)
                require(a >= 0 and a - 2 * b <= 0, "AD whole-family lower-world bound retained")
                if t == u:
                    require(a == b == 0, "AD whole-family upper world identically zero")
        if decision["policy"]["kind"] == "point":
            require_same_wire(
                values, previous["point_world_excesses"], "AD whole point-world vector recovered"
            )
            require_same_wire(
                wire(max(fraction(v) for v in values)),
                previous["upper_bound_value"],
                "AD unchanged case-average bound recovered",
            )
    return {
        **dict.fromkeys(AE_CONTROLS, True),
        "origin_authenticated_by_generic_API": False,
...
def check_case(case, adcase, zcase, accase, aacase, abcase, letters):
    projected = project_case(adcase, zcase)
    controls = check_analysis(case["analysis"], adcase, zcase, accase, aacase, abcase, letters)
    require_same_wire(
        case,
        {
            **projected,
            "analysis": expected_analysis(projected["problem"]),
            "producer_controls": controls,
        },
        "complete native AE case and baseline",
    )
    return controls
...
def run_suite():
    ...
    primary = load("qr05ae_primary", "safety.py")
    reference = load("qr05ae_reference", "reference_qr05ae.py")
    cases = []
    for ad, z, ac, aa, ab in zip(
        data["ad"]["cases"],
        data["z"]["cases"],
        data["ac"]["cases"],
        data["aa"]["cases"],
        ...
        case = project_case(ad, z)
        p = case["problem"]
        before = canonical(p)
        left = primary.analyze(p)
        require(canonical(p) == before, "primary changed AE input")
        right = reference.analyze(p)
        require(canonical(p) == before, "reference changed AE input")
        require_same_wire(left, right, "two complete independent history diagnostics")
        controls = check_analysis(left, ad, z, ac, aa, ab, letters)
    ...
    for kind in range(8):
        ...
        if kind == 0:
            ...
            bad["histories"][0]["weight"] = [0, 1]
        elif kind == 2:
            bad["histories"][0]["belief_id"] = False
        elif kind == 3:
            bad["profiles"][15]["coefficients"].pop()
        elif kind == 4:
            bad["profiles"][0]["coefficients"][0]["quadratic_coefficient"] = [-1, 1]
        elif kind == 5:
            bad["profiles"][15]["coefficients"][0]["half_linear_coefficient"] = [3, 1]
        elif kind == 6:
            bad["decisions"][0]["policy"] = {"kind": "point", "weight": 0.5}
        else:
            bad["decisions"][0]["policy"] = {"kind": "interval", "lower": [0, 1], "upper": [1, 2]}
        invalid.append(bad)
    rejected = 0
    for engine in (primary, reference):
        for bad in invalid:
            try:
                ...
                raise ValueError("malformed AE input accepted")
    require(rejected == 16, "all malformed AE public controls rejected")
    totals = {"cases": 6, "analyze_calls": 12, "invalid_analyze_calls_rejected": 16}
    totals.update(
        {k: sum(c["analysis"]["counts"][k] for c in cases) for k in cases[0]["analysis"]["counts"]}
    )
    return {
        "producer": {"artifact": AD_PRIOR, "bytes": AD_BYTES, "sha256": AD_SHA},
        "cases": cases,
