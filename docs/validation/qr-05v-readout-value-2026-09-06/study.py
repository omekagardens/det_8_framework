"""QR-05V readout-value audit, capture, and read-only replay.
...
U producer/raw inputs are declared dependencies; no prior executor is imported.
"""
...
SCHEMA = "det8-qr05v-results-v1"
BASE_COMMIT = "7f75bd1b2c518ba8f5f200bde493f6de3daac2a0"
SOURCES = (
    "README.md",
    "readout_value.py",
    "reference_qr05v.py",
...
def analyze_raw_belief(domain, entry, raw_posterior, rate, K, future):
...
def compile_analysis(domain, problem, rows):
...
def expected_analysis(raw_model, case, domain=None):
    ...
    for entry, received in zip(problem["beliefs"], source["histories"], strict=True):
        ...
        rows.append(
            analyze_raw_belief(domain, entry, normalized(raw_joint), problem["rate"], K, future)
        )
    analysis = compile_analysis(domain, problem, rows)
    require_same_wire(
...
def run_suite():
    direct = load("_qr05v_primary", "readout_value.py")
    reference = load("_qr05v_reference", "reference_qr05v.py")
    data = fixtures()
    ...
    for case in data["cases"]:
        ...
        require_same_wire(a, b, "independent V routes differ")
    ...
    for route in (direct, reference):
        for bad in (
            ...
            try:
                ...
                raise ValueError("malformed V input accepted")
    totals = {
        ...
        "readout_cells": {
            name: sum(c["analysis"]["counts"]["readout_cells"][name] for c in results)
            for name in NAMES
        },
        "readout_prediction_atoms": {
            name: sum(c["analysis"]["counts"]["readout_prediction_atoms"][name] for c in results)
            for name in NAMES
        },
        "strict_gain_beliefs": {
            name: sum(c["analysis"]["counts"]["strict_gain_beliefs"][name] for c in results)
            for name in NAMES
        },
        "positive_oracle_residual_beliefs": sum(
