"""QR-05DD order-only uniform bound driver.
...
SOURCE_PATHS = ("README.md", "protocol.json", "primary.py", "reference.py",
                "study.py", "test_qr05dd.py")
FIXTURE_PATHS = ("../../../det8/models/order_count_geometry.py",)
CAPTURE_SCHEMA = "qr05dd-capture-v1"
FREEZE_SCHEMA = "qr05dd-source-freeze-v1"
...
def _run():
    ...
    primary = load_module("_qr05dd_primary", HERE / "primary.py")
    reference = load_module("_qr05dd_reference", HERE / "reference.py")
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="QR-05DD order-only uniform bound")
