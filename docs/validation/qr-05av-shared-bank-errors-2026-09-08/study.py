"""QR-05AV shared-bank error orchestration."""
...
SCHEMA = "det8-qr05av-results-v1"
SOURCES = ("README.md", "kernel.py", "reference.py", "study.py", "test_qr05av.py")
AU = HERE.parent / "qr-05au-source-independent-receiver-2026-09-08/results.json"
AU_ID = {
    "bytes": 1073452,
    "sha256": "25e2fabfcf9e18df3ee3bf41fcac15a8b18cf3be1c4e14858be87316b48257ad",
}
...
def prior_document():
    require(ident(AU) == AU_ID, "AU artifact changed")
    return json.loads(plain_bytes(AU))


def identities():
    previous = prior_document()
    prior = {str(AU.relative_to(HERE.parent)): AU_ID}
    ...
    for name, want in previous["source_ledger"].items():
        ancestors[str((AU.parent / name).relative_to(HERE.parent))] = want
...
def project_inputs(au_families):
    require(type(au_families) is list and len(au_families) == 2, "AU family inventory")
    require([f["problem"]["family"] for f in au_families] == list(FAMILIES), "AU family order")
    problems = []
    for family in au_families:
        old = family["problem"]
        verify_suite(family["decoder"]["target_rows"], list(range(64)))
        p = {
            "family": old["family"],
            "target_labels": old["target_labels"],
            "interface": family["interface"],
            "geometric": old["geometric"],
            "decoder": family["decoder"]["matrix"],
            "primitive_map": [[[int(i == j), 1] for j in range(48)] for i in range(48)],
            "target_scales": [[1, 1] for _ in range(64)],
        }
        require(
...
def load_engine(name):
    ...
    spec = importlib.util.spec_from_file_location("qr05av_" + name, HERE / filename)
...
def controls(families):
    ...
    for family in families:
        ...
        q, r = len(problem["geometric"]), len(problem["geometric"][0])
        s, p = len(problem["interface"]), len(problem["primitive_map"][0])
        require(
            family["enclosure"]["common_bank_feasibility_tested"] is False,
            "enclosure witnesses have no asserted primitive preimage",
        )
        for name, sign_width in (("shared", p), ("enclosure", s)):
            result = family[name]
            require(
                type(result["row_gains"]) is list and len(result["row_gains"]) == q, "all row gains"
            )
            index_rows(result["max_rows"], q, True)
            for i in result["max_rows"]:
                verify_suite(result["row_gains"][i], result["max_gain"])
            witnesses = result["witnesses"]
            require(type(witnesses) is list and len(witnesses) == q, "all row witnesses")
            for i, row in enumerate(witnesses):
                ...
                require(
                    ...
                    "original target witness indexing",
                )
                signs = row["signs"]
                require(
                    type(signs) is list
                    ...
                    "native witness signs",
                )
                widths = {"observed_error": s, "decoded_error": q, "normalized_error": q}
                if name == "shared":
                    widths.update({"primitive": p, "bank_error": r, "direct_error": q})
        ...
        require(
            sorted(comparison["strict_rows"] + comparison["tied_rows"]) == list(range(q)),
            "complete disjoint gain partition",
        )
        index_rows(comparison["max_gap_rows"], q, True)
