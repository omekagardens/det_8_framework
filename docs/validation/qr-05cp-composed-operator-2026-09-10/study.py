"""QR-05CP composed T7+T5 operator driver."""
...
SOURCE_PATHS = ("README.md", "protocol.json", "primary.py", "reference.py", "study.py", "test_qr05cp.py")
CAPTURE_SCHEMA = "qr05cp-capture-v1"
FREEZE_SCHEMA = "qr05cp-source-freeze-v1"
...
def analyze_native():
    ...
    primary = load_module("_qr05cp_primary", HERE / "primary.py")
    reference = load_module("_qr05cp_reference", HERE / "reference.py")
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="QR-05CP composed T7+T5 operator")
