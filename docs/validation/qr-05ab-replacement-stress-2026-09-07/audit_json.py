"""Independent stdlib JSON-only QR-05AB replacement-mechanism audit.
...
def audit_capture(path=None):
    ...
    doc = json.loads(data)
    eq(canon(doc), data, "canonical AB envelope bytes")
    fields(doc, "schema_version runtime source_ledger prior_artifacts suite", "AB envelope")
    native_eq(doc["schema_version"], "det8-qr05ab-results-v1", "AB schema")
    ...
    native_eq(sorted(doc["source_ledger"]), sorted(names), "seven exact AB source filenames")
    inherited = []
    ...
    aadoc, zdoc, ydoc, xdoc, wdoc = inherited
    native_eq(
    ...
    expected_priors = {**aadoc["prior_artifacts"], AA_PRIOR: AA_ID}
    native_eq(len(expected_priors), 32, "32-prior lineage size")
    native_eq(doc["prior_artifacts"], expected_priors, "exact AB inherited artifact lineage")
    ...
    aasuite, zsuite, ysuite, xsuite, wsuite, suite = (
        aadoc["suite"],
        zdoc["suite"],
        ydoc["suite"],
        xdoc["suite"],
    ...
    for case, aacase, zcase, ycase, xcase, wcase in zip(
        suite["cases"],
        aasuite["cases"],
        zsuite["cases"],
        ysuite["cases"],
    ...
    for current in (doc, aadoc, zdoc, ydoc, xdoc):
        runtime_metadata(current["runtime"])
    for source, record in frozen.items():
        native_eq(ident(source), record, "after source/prior identity " + str(source))
    native_eq(ident(capture), before, "read-only AB artifact identity")
    ...
    return {
        ...
        "immutable_prior_artifacts": 32,
        "source_content_used_only_for_identity": True,
        "executor_runner_test_imports": False,
        "complete_native_canonical_X_Y_Z_AA_and_AB_suites_checked": True,
