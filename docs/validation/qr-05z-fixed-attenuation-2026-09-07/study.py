"""QR-05Z fixed-mixture capture and read-only replay.

Y laws and policies are pinned dependencies. The runner carries its own Y
coordinate scorer and adds an independent Z score check; the JSON-only audit
authenticates their raw-law bridge without importing executors or this file.
"""
...
SCHEMA = "det8-qr05z-results-v1"
BASE_COMMIT = "67468abfa9fc8e20622651f0cb1fb33ac1e903d5"
SOURCES = (
    "README.md",
    "attenuation.py",
    "reference_qr05z.py",
...
def require_wire(value):
    ...
    safely above valid Z schemas, including evidence envelopes.
    """
...
def expected_y_analysis(problem):
    rows = []
...
Z_SCALARS = ("forecast_risk", "regret", "gain_over_coarse", "gain_over_base")
Z_CHECKS = (
    "normalization",
    "score_decomposition",
...
def fixtures():
    raw = plain_bytes(HERE.parent / Y_PRIOR)
    require(len(raw) == Y_BYTES and digest(raw) == Y_SHA, "pinned Y artifact")
    doc = json.loads(raw)
    require(canonical(doc) == raw, "canonical Y envelope")
    require_wire(doc["suite"])
    expected = {**doc["prior_artifacts"], Y_PRIOR: {"bytes": Y_BYTES, "sha256": Y_SHA}}
    require_same_wire(expected, priors(), "exact inherited 30-artifact inventory")
    require(len(doc["suite"]["cases"]) == 6, "six fixed Y cases")
    return doc["suite"]
...
def case_problem(case):
    return {
        ...
        "experiment": copy.deepcopy(case["problem"]),
    }


def z_checks(distance, bayes, base_risk, coarse_risk, blends, equals):
    require(0 <= distance <= 2, "endpoint forecast distance")
    require(len(blends) == len(equals) == 3, "three fixed blends")
    for index, blend in enumerate(blends):
        require_same_wire(blend["retained_weight"], RETAINED_WEIGHTS[index], "fixed weight")
        a = fraction(blend["retained_weight"])
        risk, regret, gain, base_gain = (fraction(blend[key]) for key in Z_SCALARS)
        require(0 <= risk <= 2 and 0 <= regret <= 2, "blend score interval")
        require(risk == bayes + regret, "Z Bayes decomposition")
        average = (1 - a) * coarse_risk + a * base_risk
        require(risk == average - a * (1 - a) * distance, "quadratic risk identity")
        require(gain == coarse_risk - risk, "coarse gain sign")
        require(
            gain == a * (coarse_risk - base_risk) + a * (1 - a) * distance,
            "quadratic gain identity",
        )
        require(
            base_gain == base_risk - risk == gain - (coarse_risk - base_risk),
            "base versus coarse gain identity",
        )
        require(risk <= average, "convex endpoint-average bound")
        require((regret == 0) == equals[index], "positive-weight full-law equality")
    require(fraction(blends[0]["forecast_risk"]) == coarse_risk, "coarse endpoint risk")
    require(fraction(blends[2]["forecast_risk"]) == base_risk, "Y endpoint risk")
    return {key: True for key in Z_CHECKS}


def expected_analysis(problem):
    """Independent coordinate-loss oracle, not the public input validator."""
    require_wire(problem)
    require(
        type(problem) is dict and set(problem) == {"schema_version", "family", "experiment"},
        "Z wrapper fields",
    )
    require(
        problem["schema_version"] == "det8-qr05z-problem-v1"
        and problem["family"] == "qr05z_fixed_attenuation",
        "Z wrapper identifiers",
    )
    baseline = expected_y_analysis(problem["experiment"])
    rows = []
    blend_atoms = mixture_terms = distance_terms = scoring_terms = 0
    for base_row in baseline["beliefs"]:
        pairs = []
        for base_pair in base_row["pairs"]:
            cells = []
            weighted_distance = F(0)
            totals = [{key: F(0) for key in Z_SCALARS} for _ in RETAINED_WEIGHTS]
            for base_cell in base_pair["cells"]:
                p, f, g = (
                    law(base_cell[key])
                    for key in ("actual_prediction", "forecast", "coarse_forecast")
                )
                mass = fraction(base_cell["actual_probability"])
                support = sorted(f.keys() | g.keys())
                distance = sum(((f.get(q, F(0)) - g.get(q, F(0))) ** 2 for q in support), F(0))
                distance_terms += len(f) + len(g)
                blends = []
                for index, wire_weight in enumerate(RETAINED_WEIGHTS):
                    a = fraction(wire_weight)
                    h = {q: (1 - a) * g.get(q, F(0)) + a * f.get(q, F(0)) for q in support}
                    h = {q: probability for q, probability in h.items() if probability}
                    require(sum(h.values(), F(0)) == 1, "normalized positive mixture")
                    mixture_terms += len(f) + len(g)
                    scoring_terms += len(p) + len(h)
                    blend_atoms += len(h)
                    bayes, risk, regret = coordinate_score(p, h)
                    require(bayes == fraction(base_cell["bayes_risk"]), "same actual Bayes law")
                    gain = fraction(base_cell["coarse_risk"]) - risk
                    base_gain = fraction(base_cell["forecast_risk"]) - risk
                    scalar = {
                        "forecast_risk": risk,
                        "regret": regret,
                        "gain_over_coarse": gain,
                        "gain_over_base": base_gain,
                    }
                    for key, value in scalar.items():
                        totals[index][key] += mass * value
                    forecast = [
                        {"value": list(q), "probability": fq(probability)}
                        for q, probability in sorted(h.items())
                    ]
                    if index in (0, 2):
                        require_same_wire(
                            forecast,
                            base_cell["coarse_forecast" if index == 0 else "forecast"],
                            "complete endpoint forecast",
                        )
                    if base_cell["used_fallback"]:
                        require(distance == 0, "fallback endpoints coincide")
                        require_same_wire(forecast, base_cell["forecast"], "fallback unchanged")
                    blends.append(
                        {
                            "retained_weight": copy.deepcopy(wire_weight),
                            "forecast": forecast,
                            **{
                                key: fq(value, key.startswith("gain_"))
                                for key, value in scalar.items()
                            },
                            "predictions_equal": p == h,
                        }
                    )
                z_checks(
                    distance,
                    fraction(base_cell["bayes_risk"]),
                    fraction(base_cell["forecast_risk"]),
                    fraction(base_cell["coarse_risk"]),
                    blends,
                    [b["predictions_equal"] for b in blends],
                )
                cells.append(
                    {
                        "value": copy.deepcopy(base_cell["value"]),
                        "forecast_distance": fq(distance),
                        "blends": blends,
                    }
                )
                weighted_distance += mass * distance
            blends = [
                {
                    "retained_weight": copy.deepcopy(RETAINED_WEIGHTS[index]),
                    **{key: fq(value, key.startswith("gain_")) for key, value in total.items()},
                }
                for index, total in enumerate(totals)
            ]
            checks = z_checks(
                weighted_distance,
                fraction(base_pair["x_scores"]["bayes_risk"]),
                fraction(base_pair["completed_forecast_risk"]),
                fraction(base_pair["coarse_forecast_risk"]),
                blends,
                [all(c["blends"][index]["predictions_equal"] for c in cells) for index in range(3)],
            )
            pairs.append(
                {
                    "actual_index": base_pair["actual_index"],
                    "assumed_index": base_pair["assumed_index"],
                    "cells": cells,
                    "forecast_distance": fq(weighted_distance),
                    "blends": blends,
                    "checks": checks,
                }
            )
        rows.append(
            {
                "belief_id": base_row["belief_id"],
                "weight": copy.deepcopy(base_row["weight"]),
                "pairs": pairs,
            }
        )
    aggregate = []
    weights = [fraction(row["weight"]) for row in rows]
    require(sum(weights, F(0)) == 1, "unchanged history likelihoods")
    for k, base_pair in enumerate(baseline["aggregate"]["pairs"]):
        distance = sum(
            (
                w * fraction(row["pairs"][k]["forecast_distance"])
        ...
        for index, wire_weight in enumerate(RETAINED_WEIGHTS):
            scalar = {
                key: sum(
                    (
                        w * fraction(row["pairs"][k]["blends"][index][key])
                        for w, row in zip(weights, rows, strict=True)
                    ),
                    F(0),
                )
                for key in Z_SCALARS
            }
            blends.append(
                {
                    "retained_weight": copy.deepcopy(wire_weight),
                    **{key: fq(value, key.startswith("gain_")) for key, value in scalar.items()},
                }
            )
        checks = z_checks(
            distance,
            ...
            blends,
            [
                all(fraction(row["pairs"][k]["blends"][index]["regret"]) == 0 for row in rows)
    ...
    specs = {
        "positive_regret": ("regret", lambda v: v > 0),
        "better_than_coarse": ("gain_over_coarse", lambda v: v > 0),
        "equal_to_coarse": ("gain_over_coarse", lambda v: v == 0),
        "worse_than_coarse": ("gain_over_coarse", lambda v: v < 0),
        "better_than_base": ("gain_over_base", lambda v: v > 0),
        "equal_to_base": ("gain_over_base", lambda v: v == 0),
        "worse_than_base": ("gain_over_base", lambda v: v < 0),
    }
    counts = {
        "beliefs": len(rows),
        ...
        "blend_scoring_terms": scoring_terms,
        "baseline_scoring_terms": baseline["counts"]["completed_scoring_terms"]
        + baseline["counts"]["coarse_scoring_terms"],
    ...
    for name, (key, predicate) in specs.items():
        counts[name + "_beliefs"] = [
            sum(predicate(fraction(row["pairs"][k]["blends"][b][key])) for row in rows)
            for k in range(16)
            for b in range(3)
        ]
        witnesses["first_" + name] = [
            next(
                (
                    row["belief_id"]
                    for row in rows
                    if predicate(fraction(row["pairs"][k]["blends"][b][key]))
                ),
                None,
            )
            for k in range(16)
    ...
    return {
        "input_sha256": digest(canonical(problem)),
        "levels": copy.deepcopy(LEVELS),
        "retained_weights": copy.deepcopy(RETAINED_WEIGHTS),
        "baseline": baseline,
        "beliefs": rows,
        "aggregate": {"total_weight": [1, 1], "pairs": aggregate},
        "witnesses": witnesses,
...
def producer_controls(analysis, case):
    require_same_wire(analysis["baseline"], case["analysis"], "entire pinned Y baseline preserved")
    require_same_wire(analysis["retained_weights"], RETAINED_WEIGHTS, "fixed nonselected weights")
    for row, base_row in zip(analysis["beliefs"], analysis["baseline"]["beliefs"], strict=True):
        for k, (pair, base_pair) in enumerate(zip(row["pairs"], base_row["pairs"], strict=True)):
            distance = fraction(pair["forecast_distance"])
            for cell, base_cell in zip(pair["cells"], base_pair["cells"], strict=True):
                require_same_wire(cell["value"], base_cell["value"], "same report order")
                if k // 4 == 3:
                    require_same_wire(
                        base_cell["actual_prediction"],
                        base_cell["coarse_forecast"],
                        "W full replacement actual future is coarse",
                    )
                if k % 4 == 3 or base_cell["used_fallback"]:
                    for blend in cell["blends"]:
                        require_same_wire(
                            blend["forecast"],
                            base_cell["coarse_forecast"],
                            "W uninformative/fallback forecast unchanged",
                        )
            for blend in pair["blends"]:
                a = fraction(blend["retained_weight"])
                if k // 4 == 3:
                    require(
                        fraction(blend["gain_over_coarse"]) == -a * a * distance,
                        "W full replacement quadratic penalty",
                    )
                if k // 4 == k % 4:
                    require(
                        fraction(blend["regret"]) == (1 - a) ** 2 * distance,
                        "W correctly specified quadratic regret",
                    )
                if k % 4 == 3:
                    require(
                        distance == 0 and blend["gain_over_coarse"] == [0, 1],
                        "W assumed full replacement equals coarse",
                    )
    return {
...
def check_analysis(analysis, case):
    ...
    require(len(candidates) == 1, "unknown Y case")
    require_same_wire(case, candidates[0], "Y producer case differs")
    expected = expected_analysis(case_problem(case))
    require_same_wire(analysis, expected, "complete independent Z scoring audit differs")
    producer_controls(analysis, case)
    return {"analysis_sha256": digest(canonical(expected)), "complete_native_output_equal": True}


def run_suite():
    primary = load("_qr05z_primary", "attenuation.py")
    reference = load("_qr05z_reference", "reference_qr05z.py")
    cases = []
    for case in fixtures()["cases"]:
        ...
        require_same_wire(a, b, "independent Z routes differ")
        require_same_wire(a, expected_analysis(problem), "independent runner Z score differs")
    ...
    for module in (primary, reference):
        for bad in (
            ...
            try:
                ...
                raise ValueError("malformed Z wrapper accepted")
    require(invalid == 16, "public malformed inventory")
    totals = {"cases": 6, "analyze_calls": 12, "invalid_analyze_calls_rejected": invalid}
    for key, value in cases[0]["analysis"]["counts"].items():
        totals[key] = (
            [sum(case["analysis"]["counts"][key][k] for case in cases) for k in range(len(value))]
            if type(value) is list
            else sum(case["analysis"]["counts"][key] for case in cases)
        )
    return {
        "producer": {"artifact": Y_PRIOR, "bytes": Y_BYTES, "sha256": Y_SHA},
        "cases": cases,
