"""QR-05DK no-go direction driver."""
...
SOURCE_PATHS = ("README.md", "protocol.json", "primary.py", "reference.py",
                "study.py", "test_qr05dk.py")
FIXTURE_PATHS = ()
CAPTURE_SCHEMA = "qr05dk-capture-v1"
FREEZE_SCHEMA = "qr05dk-source-freeze-v1"
...
def _run():
    ...
    primary = load_module("_qr05dk_primary", HERE / "primary.py")
    reference = load_module("_qr05dk_reference", HERE / "reference.py")
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="QR-05DK no-go direction")
