...
def _p99(values):
    ...
    return ordered[min(int(0.01 * len(ordered)), len(ordered) - 1)]
...
def _bundle(module, n, seed, area):
    ...
    count_time = [[ell * math.sqrt(counts[i][j] + 1) if prec[i][j] else 0.0 for j in range(n)] for i in range(n)]
...
def build_report(protocol):
    ...
    for n in protocol["n_scale"]:
        chain_upper, chain_lower, chain_p99, count_upper, count_p99 = [], [], [], [], []
        for s in protocol["seeds"]:
            ...
            chain_p99.append(_p99([abs(chain[i][j] - c_chain * tau[i][j])
            ...
            count_p99.append(_p99([abs(count[i][j] - c_count * tau[i][j])
                                   for i in range(n) for j in range(n) if tau[i][j] > 0]))
        ell = math.sqrt(area / n)
    ...
    for key in ("chain_lower", "chain_upper", "chain_p99", "count_upper", "count_p99"):
    ...
    flags = {
        ...
        "chain_bulk_decays_faster_than_chain_max": bool(decay["chain_p99"] > decay["chain_upper"]),
        "count_max_decays_faster_than_chain_max": bool(decay["count_upper"] > decay["chain_upper"]),
        "count_bulk_decays_faster_than_chain_bulk": bool(decay["count_p99"] > decay["chain_p99"]),
    ...
    return {
        ...
        "verdict": (
            ...
            "estimator, not a support or convention defect. The chain bracket therefore does "
            "NOT close and chain-based convergence is not established. The order+count "
            f"(T7-native) reconstruction decays faster (ceiling ~N^-{decay['count_upper']:.2f}, "
            f"bulk 99th percentile ~N^-{decay['count_p99']:.2f}, consistent with Poisson "
            f"counting) and falls below the chain ceiling from N={crossover}. The obstruction "
            "is therefore estimator-specific: the convergence question depends on which "
            "discrete time-separation reconstruction is used, not on the directed-distance "
            "convention (already corrected by QR-05CZ). No metric, continuum, curvature or "
            "gravity claim."),
    }
