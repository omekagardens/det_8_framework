"""Independent source-law and grid-worklist oracle for QR-05L.

Utility lineage: the separately carried QR-05K reference implementation.
Runtime imports no prior executor, artifact, or primary route.  Source-chain
indicators and interpolated count-law moments supply D/B, while fine transition
coefficients come from exact rational interpolation rather than binomial expansion.

An exact-grid splitter worklist constructs the stable partition independently
of the separately emitted synchronous-grid round certificate.  Their canonical
terminal memberships must agree.  Only analyze(problem) is advertised; closure
and coarseness refer to this bounded supplied observation family, not physics,
universal trace sufficiency, or minimal storage.
"""
...
def _parse(problem):
    ...
    _require(
        ...
        and problem["schema_version"] == "det8-qr05l-problem-v1",
        "reference schema is unsupported",
    )
    _require(
        type(problem["family"]) is str and problem["family"] == "qr05l_adversarial_iid",
...
def analyze(problem):
    """Return only the frozen adversarial domain and stable-refinement certificate."""
    _parse(problem)
