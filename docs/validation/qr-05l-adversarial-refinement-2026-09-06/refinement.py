"""QR-05L observed-order adversarial closure and stable partition refinement.

Observed-chain, binomial, and quotient utilities descend from this route's
QR-05K primary source, copied locally rather than imported at runtime.
Synchronous coefficient-row refinement is constructed on the expanded domain.
...
def analyze(problem):
    ...
    if (
        ...
        or problem["schema_version"] != "det8-qr05l-problem-v1"
        or type(problem.get("family")) is not str
        or problem["family"] != "qr05l_adversarial_iid"
    ):
        raise ValueError("expected the exact QR-05L qr05l_adversarial_iid problem")
