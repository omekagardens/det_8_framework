"""QR-05CA synthetic pipeline rehearsal driver.

Loads the fixed protocol, runs the independent primary and reference routes,
compares them natively, and (in ``main``) writes the create-only capture and
source freeze. No randomness, no floats, no network.
"""
...
SOURCE_PATHS = ("README.md", "protocol.json", "primary.py", "reference.py", "study.py", "test_qr05ca.py")
CAPTURE_SCHEMA = "qr05ca-capture-v1"
FREEZE_SCHEMA = "qr05ca-source-freeze-v1"
...
def analyze_native():
    ...
    primary = load_module("_qr05ca_primary", HERE / "primary.py")
    reference = load_module("_qr05ca_reference", HERE / "reference.py")
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="QR-05CA synthetic pipeline rehearsal")
    parser.add_argument("--freeze", action="store_true", help="write source-freeze.json")
    parser.add_argument("--run", action="store_true", help="write results.json")
    args = parser.parse_args(argv)
    if not (args.freeze or args.run):
        args.freeze = args.run = True

    protocol = load_protocol()
    limit = protocol["limits"]["source_bytes"]
    for name in SOURCE_PATHS:
