"""QR-05O observed path feature and obstruction-targeted summary executor.

Base observed-chain, support-union, binomial, and closure utilities are carried
locally from this route's QR-05N primary, never imported at runtime.
The entire N analysis is retained; the new P4 feature depends only on observed
relations and eligible marks and is tested without automatic refinement.
...
def _path_observable(frames, states):
    """Construct T, H and its complete law before comparing existing fibers."""
    observed, classes, vector, identities = [], [], [], {}
    for state in states:
        frame = frames[state["frame_id"]]
        supports = path_supports(state["kept"], state["past"], frame["eligible"])
        count = len(supports)
        odd, even = state["color_sizes"]
        if count + state["motif_count"] > comb(odd, 2) * comb(even, 2):
            raise ValueError("induced M/T four-support disjointness bound failed")
        observed.append(
    ...
    rows = _round_pushforwards(states, vector)
    maps = [
    ...
    for state in states:
        actual = [[0] * 4 for _ in range(4)]
        for atom in state["transitions"]:
            _add_table(actual, atom["coefficients"], observed[atom["target_state"]]["path_count"])
        expected = [[0] * 4 for _ in range(4)]
        expected[2][2] = observed[state["state_id"]]["path_count"]
        maximum = max(abs(actual[i][j] - expected[i][j]) for i in range(4) for j in range(4))
        if maximum:
            raise ValueError("eligible induced-path expectation identity failed")
        update_rows.append(
            {
                "state_id": state["state_id"],
                "coefficients": actual,
    ...
    update = {"verified": True, "coefficient_cells": 16 * len(update_rows), "rows": update_rows}

    # Existing class labels are consulted only after the feature and H law exist.
    pair = [state["classes"]["pair_graded"] for state in states]
    motif_pair = [state["classes"]["motif_pair"] for state in states]
    comparison = {
        "candidate_refines_pair": _class_vector_refines(vector, pair),
        "candidate_refines_motif_pair": _class_vector_refines(vector, motif_pair),
        "motif_pair_refines_candidate": _class_vector_refines(motif_pair, vector),
        "same_memberships_as_motif_pair": vector == motif_pair,
    }
    if not comparison["candidate_refines_pair"] or not comparison["candidate_refines_motif_pair"]:
        raise ValueError("candidate discarded a required B/M distinction")
    children = {}
    for old_class, new_class in zip(motif_pair, vector, strict=True):
        children.setdefault(old_class, set()).add(new_class)
    state_atoms = sum(len(row["transitions"]) for row in rows)
    quotient_atoms = sum(len(row["transitions"]) for row in quotient)
    if (
        state_atoms > 99180
        or quotient_atoms > 99180
        or (semigroup is not None and semigroup["totals"]["coefficient_cells"] > 25390080)
    ):
        raise ValueError("path candidate resource cap exceeded")
    return {
        ...
        "counts": {
            "states": len(observed),
            "path_positive_states": sum(item["path_count"] > 0 for item in observed),
            "path_support_occurrences": sum(item["path_count"] for item in observed),
            "max_path_count": max((item["path_count"] for item in observed), default=0),
            "classes": len(classes),
            "split_motif_pair_classes": sum(len(values) > 1 for values in children.values()),
...
def analyze(problem):
    ...
    if (
        ...
        or problem["schema_version"] != "det8-qr05o-problem-v1"
        or type(problem.get("family")) is not str
        or problem["family"] != "qr05o_path_iid"
    ):
        raise ValueError("expected the exact QR-05O qr05o_path_iid problem")
    ...
    path_observable = _path_observable(frames, states)
