"""QR-05M observable realization capture and read-only replay.

Base L audit/lifecycle utilities are carried locally; prior JSON is read only
for independently reconstructed evidence comparison, never as executor input.
"""
...
SCHEMA = "det8-qr05m-results-v1"
BASE_COMMIT = "156a644350d7649c69aaf2316e78446d3fcdbed0"
SOURCES = (
    "README.md",
    "observable.py",
    "reference_qr05m.py",
...
def observed_motifs(kept, past, eligible):
    """Audit by exact equality to either complete induced relation pattern."""
    edges = {(kept[i], kept[j]) for j, row in enumerate(past) for i in row}
    present = sorted(set(kept) & set(eligible))
    result = []
    for support in combinations(present, 4):
        odd = [v for v in support if v % 2]
        even = [v for v in support if not v % 2]
        if len(odd) != 2 or len(even) != 2:
            continue
        actual = {(u, v) for u, v in edges if u in support and v in support}
        forward = {(u, v) for u in odd for v in even}
        backward = {(v, u) for u in odd for v in even}
        if actual == forward or actual == backward:
            result.append(list(support))
    return result


def check_observable(a):
    """Reconstruct motif evidence without the executor recognizers or Q* lookup."""
    require_wire(a)
    require(retained_bits(a) <= 4096, "observable component cap")
    states = a["states"]
    actual = a["observable"]
    features, identities, vector = [], {}, []
    for state in states:
        frame = a["frames"][state["frame_id"]]
        supports = observed_motifs(state["kept"], state["past"], frame["eligible"])
        features.append(
            {
                "state_id": state["state_id"],
                "motif_supports": supports,
                "motif_count": len(supports),
            }
        )
        key = canonical([frame_value(frame), state["pair_graded"], len(supports)])
        if key not in identities:
            identities[key] = len(identities)
        vector.append(identities[key])
    classes = groups_from_vector(vector)
    pushed = push_to_vector(states, vector)
    failure = None
    maps = [law_map(row["transitions"], "target_class") for row in pushed]
    for group in classes:
        ...
        for right in group["members"][1:]:
            different = [
                ...
                if maps[left].get(target, zero()) != maps[right].get(target, zero())
            ]
            if different:
                target = different[0]
                failure = {
                    ...
                    "right_state": right,
                    "target_class": target,
                    "left_coefficients": maps[left].get(target, zero()),
                    "right_coefficients": maps[right].get(target, zero()),
                }
                break
    ...
    semigroup = None if failure is not None else check_semigroup(quotient)
    expected_rows = []
    for state in states:
        table = zero()
        for atom in state["transitions"]:
            ...
            for i, j in product(range(4), repeat=2):
                table[i][j] += atom["coefficients"][i][j] * count
        expected = zero()
        expected[2][2] = features[state["state_id"]]["motif_count"]
        require_same_wire(table, expected, "motif expectation identity")
    ...
    stable = a["stabilization"]
    comparison = {
        "same_memberships_as_stable_pair": vector == stable["state_classes"],
        "candidate_refines_stable": vector_refines(vector, stable["state_classes"]),
        "stable_refines_candidate": vector_refines(stable["state_classes"], vector),
    }
    counts = {
        "states": len(states),
        "motif_positive_states": sum(f["motif_count"] > 0 for f in features),
        "motif_support_occurrences": sum(f["motif_count"] for f in features),
        "max_motif_count": max(f["motif_count"] for f in features),
        "classes": len(classes),
    ...
    require_same_wire(actual, rebuilt, "observable reconstruction")
    if comparison["same_memberships_as_stable_pair"]:
        require_same_wire(classes, stable["classes"], "actual stable memberships")
        require_same_wire(pushed, stable["rounds"][-1]["pushforward_rows"], "stable state laws")
        require_same_wire(quotient, stable["quotient_rows"], "stable quotient coefficients")
        require_same_wire(semigroup, stable["semigroup"], "stable composition certificate")
    evaluated_atoms, corner_rows = 0, 0
    for row in pushed:
        sid = row["state_id"]
        state = states[sid]
        frame = a["frames"][state["frame_id"]]
        require_same_wire(
            [
                [sum(atom["coefficients"][i][j] for atom in row["transitions"]) for j in range(4)]
                for i in range(4)
            ],
        ...
        for x, y in AUDIT_POINTS:
            ...
            require(all(p >= 0 for p in evaluated.values()), "candidate rate sign")
            require(sum(evaluated.values()) == 1, "candidate rate normalization")
            fine = {}
            for atom in state["transitions"]:
                target = states[atom["target_state"]]
                cid = vector[target["state_id"]]
                fine[cid] = fine.get(cid, F(0)) + probability(frame, state, target, x, y)
            require(evaluated == fine, "candidate row direct rate check")
            evaluated_atoms += len(evaluated)
            if x in (0, 1) and y in (0, 1):
                surviving = [
                ...
                target = next(
                    ...
                    and s["kept"] == surviving
                    and s["past"] == induced_past(state, surviving)
                )
                require(
                    {cid: p for cid, p in evaluated.items() if p}
                    == {vector[target["state_id"]]: F(1)},
                    "candidate deterministic corner",
                )
                corner_rows += 1
    return {
        "verified": True,
        "same_memberships_as_stable_pair": comparison["same_memberships_as_stable_pair"],
        "closed": failure is None,
...
def observable_control_inputs():
    """Well-formed synthetic recognizer controls, outside the certified domain."""
    forward = [(u, v) for u in (1, 3) for v in (4, 6)]
    quartet = [1, 3, 4, 6]

    def case(name, kept, edges, eligible, expected):
        relation = set(edges)
        while True:
            ...
            if closure == relation:
                break
            relation = closure
        require(all(u != v for u, v in relation), "synthetic strict order")
        positions = {v: i for i, v in enumerate(kept)}
        past = [sorted(positions[u] for u, v in relation if v == right) for right in kept]
        return {
            "name": name,
            "kept": kept,
            "past": past,
    ...
    return [
        case("forward_once", quartet, forward, quartet, [quartet]),
        case("reverse_once", quartet, [(v, u) for u, v in forward], quartet, [quartet]),
        case("missing_cross", quartet, forward[:-1], quartet, []),
        case("same_color_comparison", quartet, forward + [(1, 3)], quartet, []),
        case(
            "wrong_color_cardinality",
            [1, 3, 5, 6],
            [(u, v) for u in (1, 3) for v in (5, 6)],
            [1, 3, 5, 6],
            [],
        ),
        case(
            "wrong_layer_colors",
            [1, 2, 3, 4],
            [(u, v) for u in (1, 2) for v in (3, 4)],
            [1, 2, 3, 4],
            [],
        ),
        case("nonuniform_cross", quartet, [(1, 4), (1, 6), (4, 3), (3, 6)], quartet, []),
        case("total_order", quartet, list(pairwise(quartet)), quartet, []),
        case("fixed_corner_excluded", quartet, forward, [1, 4, 6], []),
        case("reordered_local_positions", [6, 1, 4, 3], forward, quartet, [quartet]),
        case(
            "color_preserving_relabel",
            [12, 9, 2, 5],
            [(rename[u], rename[v]) for u, v in forward],
            [2, 5, 9, 12],
            [[2, 5, 9, 12]],
        ),
        case(
        ...
        case(
            "overlapping_k23",
            [1, 3, 4, 6, 8],
            [(u, v) for u in (1, 3) for v in (4, 6, 8)],
            [1, 3, 4, 6, 8],
...
def observable_controls(direct, reference):
    result = []
    for item in observable_control_inputs():
        args = [item[key] for key in ("kept", "past", "eligible")]
        independent = observed_motifs(*args)
        primary = direct.motif_supports(*args)
        secondary = reference.motif_supports(*args)
        for value in (independent, primary, secondary):
            require_same_wire(value, item["expected_supports"], "synthetic motif support")
        result.append(
            {**item, "primary_supports": primary, "reference_supports": secondary, "verified": True}
        )
    overlap = next(item for item in result if item["name"] == "overlapping_k23")
    eligible = overlap["eligible"]
    row = zero()
    atoms = []
    for mask in range(1 << len(eligible)):
        kept = [v for bit, v in enumerate(eligible) if mask & (1 << bit)]
        past = induced_past(overlap, kept)
        values = [len(route.motif_supports(kept, past, eligible)) for route in (direct, reference)]
        require(
            values == [len(observed_motifs(kept, past, eligible))] * 2, "overlap induced counts"
        )
        odd, even = sum(v % 2 for v in kept), sum(not v % 2 for v in kept)
        coefficients = zero()
        for i in range(2 - odd + 1):
            for j in range(3 - even + 1):
                coefficients[odd + i][even + j] = (
                    (-1) ** (i + j) * comb(2 - odd, i) * comb(3 - even, j)
                )
        for i, j in product(range(4), repeat=2):
            row[i][j] += coefficients[i][j] * values[0]
        atoms.append(
            {"mask": mask, "kept": kept, "motif_count": values[0], "coefficients": coefficients}
        )
    expected = zero()
    expected[2][2] = 3
    require_same_wire(row, expected, "overlapping motif expectation")
    return {
        "recognizer_cases": result,
        "recognizer_case_count": len(result),
        "overlap_expectation": {
...
def l_bridge(a, suite):
    require_wire(a)
    old = prior_json("qr-05l-adversarial-refinement-2026-09-06/results.json")["suite"]
    projection = {key: value for key, value in a.items() if key != "observable"}
    require_same_wire(projection, old["analysis"], "whole L analysis projection")
    retained = ["audit", "refinement_audit", "trace_audit", "controls", "prior_bridge"]
    for field in retained:
        require_same_wire(suite[field], old[field], "retained L suite evidence " + field)
    return {
        "matched_states": len(a["states"]),
        "matched_aliases": sum(len(state["aliases"]) for state in a["states"]),
        "whole_analysis_projection_equal": True,
        "retained_suite_fields": retained,
        "stable_class_memberships_equal": a["observable"]["comparison"][
            "same_memberships_as_stable_pair"
        ],
        "quotient_coefficients_equal": canonical(a["observable"]["quotient_rows"])
        == canonical(a["stabilization"]["quotient_rows"]),
        "semigroup_certificate_equal": canonical(a["observable"]["semigroup"])
        == canonical(a["stabilization"]["semigroup"]),
        "prior_artifact": "qr-05l-adversarial-refinement-2026-09-06/results.json",
    }


def run_suite():
    direct = load("qr05m_capture_direct", "observable.py")
    reference = load("qr05m_capture_reference", "reference_qr05m.py")
    problem = fixtures()[0]
    a, b = direct.analyze(problem), reference.analyze(problem)
    require_same_wire(a, b, "independent complete M outputs differ")
    audit = check_analysis(a)
    refinement_audit = check_stabilization(a)
    trace = check_traces(a)
    observable_audit = check_observable(a)
    rejections = []
    for index, invalid in enumerate(invalid_fixtures()):
        ...
        for name, module in (
            ...
            try:
                ...
                raise ValueError("malformed M problem accepted")
    ...
    suite = {
        ...
        "observable_controls": observable_controls(direct, reference),
        "controls": controls(a),
        "prior_bridge": prior_bridge(a),
        "rejections": rejections,
        "research_base_commit": BASE_COMMIT,
        "claim": "An observed eligible color-layered induced K2,2 count is tested as a constructive realization of the finite stable B-refinement; full-law closure and partition agreement are checked separately from its motif-survival expectation.",
        "bounds": {
            "events_per_source": 8,
    ...
    suite["l_bridge"] = l_bridge(a, suite)
    suite["candidate_succeeded"] = bool(
        a["observable"]["closed"]
        and a["observable"]["comparison"]["same_memberships_as_stable_pair"]
    )
    suite["general_motif_expectation_basis"] = (
        "Induced support heredity and linearity of expectation under independent eligible two-color retention; overlap independence is not assumed."
    )
    suite["unseen_domain_candidate_closure_claimed"] = False
    suite["motif_is_transition_table_lookup"] = False
    ...
    suite["totals"] = {
        **a["counts"],
        ...
        "observable": a["observable"]["counts"],
        "synthetic_recognizer_cases": suite["observable_controls"]["recognizer_case_count"],
        "all_semigroup_coefficient_cells": a["counts"]["semigroup_coefficient_cells"]
        + a["stabilization"]["counts"]["semigroup_coefficient_cells"]
        + a["observable"]["counts"]["semigroup_coefficient_cells"],
