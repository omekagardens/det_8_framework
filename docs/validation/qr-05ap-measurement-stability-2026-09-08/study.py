"""QR-05AP frozen coarse-response measurement-error orchestration."""
...
SCHEMA = "det8-qr05ap-results-v1"
SOURCES = ("README.md", "kernel.py", "reference.py", "study.py", "test_qr05ap.py")
...
def prior_document():
    ...
    return json.loads(plain_bytes(AO))


def identities():
    previous = prior_document()
    prior = {str(AO.relative_to(HERE.parent)): AO_ID}
    ...
    for name, want in previous["source_ledger"].items():
        ancestors[str((AO.parent / name).relative_to(HERE.parent))] = want
...
def project_inputs(previous):
    require(type(previous) is list and len(previous) == 2, "complete AO family inventory")
    require([f["problem"]["family"] for f in previous] == list(FAMILIES), "AO family order")
    problems = []
    for old, name in zip(previous, FAMILIES, strict=True):
        p, g, m = old["problem"], old["geometry"], old["matrices"]
        ...
        ids = [c["event"] for c in p["coarse"]["cells"]]
        require([c["event"] for c in g["coarse_cells"]] == ids, "AO coarse cell order")
        require(
            len(g["coarse_tiles"]) == 8 and len(m["generators"]) == 64,
            "AO tile/generator inventory",
        )
        require(
            [o["name"] for o in m["observations"]] == ["integral", "weighted"],
            "AO observation order",
        )
        require([t["name"] for t in m["targets"]] == ["coarse", "fine", "field"], "AO target order")
        verify_suite(
            m["observations"][1]["rows"],
            [{"tile": t, "basis": j} for t in range(8) for j in range(4)],
        )
        verify_suite(m["targets"][0]["rows"], [{"first": c, "second": d} for c in ids for d in ids])
        certs = old["certificates"]
        verify_suite(
        ...
        certificate = certs[3]["certificate"]
        require(
            certificate["recoverable"] is True and certificate["collision"] is None,
            "AO weighted coarse recovery",
        )
        projected = {
            "family": name,
            ...
            "canonical": {"row_basis": certificate["row_basis"], "decoder": certificate["decoder"]},
            "geometric": m["coarse_decoder"],
        }
        problems.append(copy.deepcopy(projected))
...
def load_engine(name):
    ...
    spec = importlib.util.spec_from_file_location("qr05ap_" + name, HERE / filename)
...
def ao_bridges(families, previous):
...
def assemble_suite(families, ao_families):
    ...
    return {
        "families": families,
        "prior_bridges": ao_bridges(families, ao_families),
