"""Independent full-law moment oracle for QR-05J.

Original-source chain indicators and exact degree-bounded interpolation
reconstruct count-law polynomials locally.  Weighting their count atoms
produces raw moments; this route never enumerates chain pairs.  Complete
mean convolution produces covariance, and centered full-law moments supply
evaluated diagnostics, including the correlated corner mixture.

No primary, prior executor, or artifact is imported or read.  Only
analyze(problem) is advertised.  This is a finite supplied-law uncertainty
contract, not empirical calibration or a claim of universal moment sufficiency.
"""
...
_TARGETS = ("analytic_means", "analytic_second")
...
def _parse(problem):
    ...
    _require(
        ...
        and problem["schema_version"] == "det8-qr05j-problem-v1",
        "reference schema is unsupported",
    )
    _require(
        type(problem["family"]) is str and problem["family"] == "qr05i_chain_pairs",
...
def _evaluate(table, x, y):
    return _bounded(
        sum(
            (
                F(coefficient) * x**a * y**b
                for a, row in enumerate(table)
                for b, coefficient in enumerate(row)
            ),
            F(0),
        )
    )
...
def _weighted_moments(polynomials, probabilities):
    """Use full-law centered moments, independently of symbolic centering."""
    _require(
        sum(probabilities, F(0)) == 1 and all(value >= 0 for value in probabilities),
        "reference evaluated count law is not a probability law",
    )
    means = [
        _bounded(
            sum(
                (
                    probability * atom["counts"][q]
                    for atom, probability in zip(polynomials, probabilities, strict=True)
                ),
                F(0),
            )
        )
    ...
    raw = [
        [
            _bounded(
                sum(
                    (
                        probability * atom["counts"][q] * atom["counts"][r]
                        for atom, probability in zip(polynomials, probabilities, strict=True)
                    ),
                    F(0),
                )
        ...
        [
            _bounded(
                sum(
                    (
                        probability
                        * (atom["counts"][q] - means[q])
                        * (atom["counts"][r] - means[r])
                        for atom, probability in zip(polynomials, probabilities, strict=True)
                    ),
                    F(0),
                )
    ...
    _require(
        all(
            covariance[q][r] == raw[q][r] - means[q] * means[r] for q in range(4) for r in range(4)
        ),
        "reference centered and raw evaluated moments disagree",
    )
    _require(
        means[0] == 1 and all(covariance[0][q] == covariance[q][0] == 0 for q in range(4)),
        "reference constant count has nonzero covariance",
    )
    return means, raw, covariance


def _moment_predictions(polynomials, means, raw, covariance):
    predictions = {}
    alpha = [F((-1) ** q, 2 ** (q + 1) * 12**q) for q in range(4)]
    corners = ((F(0), F(0)), (F(1), F(0)), (F(0), F(1)), (F(1), F(1)))
    corner_probabilities = [
        [_evaluate(atom["coefficients"], x, y) for atom in polynomials] for x, y in corners
    ]
    _require(
        all(sum(p > 0 for p in row) == 1 and sum(row, F(0)) == 1 for row in corner_probabilities),
        "reference corner law is not deterministic",
    )
    for policy in ("iid_half", "iid_color", "block_coin"):
        if policy == "block_coin":
            probabilities = [
                sum((row[index] for row in corner_probabilities), F(0)) / 4
                for index in range(len(polynomials))
            ]
        else:
            x, y = (F(1, 2), F(1, 2)) if policy == "iid_half" else (F(1, 3), F(2, 3))
            probabilities = [_evaluate(atom["coefficients"], x, y) for atom in polynomials]
        m, r, k = _weighted_moments(polynomials, probabilities)
        if policy == "block_coin":
            expected_m = [
                sum((_evaluate(table, x, y) for x, y in corners), F(0)) / 4 for table in means
            ]
            expected_r = [
                [
                    sum((_evaluate(raw[q][r_index], x, y) for x, y in corners), F(0)) / 4
            ...
            _require(
                all(
                    _evaluate(covariance[q][r_index], x, y) == 0
                    for q in range(4)
                    for r_index in range(4)
                    for x, y in corners
                ),
                "reference deterministic corners have conditional covariance",
            )
            expected_k = [
                [
                    expected_r[q][r_index] - expected_m[q] * expected_m[r_index]
                    for r_index in range(4)
                ]
                for q in range(4)
            ]
        else:
            expected_m = [_evaluate(table, x, y) for table in means]
            expected_r = [
                [_evaluate(raw[q][r_index], x, y) for r_index in range(4)] for q in range(4)
            ]
            expected_k = [
                [_evaluate(covariance[q][r_index], x, y) for r_index in range(4)] for q in range(4)
            ]
        _require(
            m == expected_m and r == expected_r and k == expected_k,
            "reference law and polynomial/mixture evaluated moments disagree",
        )
        predictions[policy] = {
            "count_mean": _vector(m),
            "count_second_moment": [_vector(row) for row in r],
            "count_covariance": [_vector(row) for row in k],
            "coefficient_mean": _vector([alpha[q] * m[q] for q in range(4)]),
            "coefficient_covariance": [
                _vector([alpha[q] * alpha[r_index] * k[q][r_index] for r_index in range(4)])
                for q in range(4)
            ],
        }
    return predictions
...
def _moment_summary(polynomials, means, color_sizes, chain_counts):
    """Weight complete count atoms, without constructing a chain-pair inventory."""
    raw = [
        [
            [
                [
    ...
    for q in range(4):
        for r in range(4):
            _require(
                all(value >= 0 for row in raw[q][r] for value in row),
                "reference raw-moment coefficients are not nonnegative integers",
            )
            _require(raw[q][r] == raw[r][q], "reference raw-moment tensor lacks symmetry")
            _require(
                sum(sum(row) for row in raw[q][r]) == chain_counts[q] * chain_counts[r],
                "reference raw-moment total disagrees with complete observed counts",
            )
            _require(
                all(
                    raw[q][r][a][b] == 0
                    for a in range(4)
                    for b in range(4)
                    if a > color_sizes[0] or b > color_sizes[1] or a + b > q + r
                ),
                "reference raw-moment degree bound failed",
            )
            for a in range(4):
                for b in range(4):
                    covariance[q][r][a][b] += raw[q][r][a][b]
                    if not means[q][a][b]:
                        continue
                    for c in range(4):
                        for d in range(4):
                            covariance[q][r][a + c][b + d] -= means[q][a][b] * means[r][c][d]
            _require(
                all(
                    covariance[q][r][a][b] == 0
                    for a in range(7)
                    for b in range(7)
                    if a > 2 * color_sizes[0] or b > 2 * color_sizes[1] or a + b > q + r
                ),
                "reference covariance degree bound failed",
            )
    _require(raw[0] == means, "reference q=0 raw-moment row does not recover the means")
    _require(
        all(covariance[q][r] == covariance[r][q] for q in range(4) for r in range(4)),
        "reference covariance coefficients lack symmetry",
    )
    _require(
        all(value == 0 for q in range(4) for row in covariance[0][q] for value in row),
        "reference constant-count covariance polynomial is nonzero",
    )
    return raw, covariance, _moment_predictions(polynomials, means, raw, covariance)
...
def _population_states(past, fixed, eligible, inverse):
    ...
    for mask, observation in enumerate(all_observations):
        ...
        raw, covariance, predictions = _moment_summary(
            polynomials, means, sizes, observation["chain_counts"]
        )
        states.append(
            {
                "mask": mask,
...
def _candidate_keys(base, state):
    ...
    return {
        ...
        "pair_graded": [*base, token, state["pair_graded"]],
    }


def _target_signatures(state):
    return state["mean_polynomials"], state["pair_graded"]
...
def analyze(problem):
    """Return the bounded uncertainty summary and same-domain predictive partitions."""
    _parse(problem)
    ...
    _require(target_refinement[1][0], "reference second moments fail to refine their means")
    for target in _TARGETS:
        _require(
    ...
    pair_forward, pair_backward = {}, {}
    _require(
        assessments["pair_graded"]["analytic_second"]["sufficient"],
        "reference pair summary fails to determine second moments",
    )
    for history in histories:
        candidate = history["candidate_classes"]["pair_graded"]
        if candidate is None:
            continue
        target = history["target_classes"]["analytic_second"]
        _require(
            candidate not in pair_forward or pair_forward[candidate] == target,
            "reference pair-summary class splits across second moments",
        )
        _require(
            target not in pair_backward or pair_backward[target] == candidate,
            "reference second-moment class splits across pair summaries",
        )
        pair_forward[candidate], pair_backward[target] = target, candidate
    states = [state for case in cases for state in case["states"]]
    pair_instances = sum(sum(state["chain_counts"]) ** 2 for state in states)
    ...
    result = {
        ...
        "counts": {
            ...
            "pair_coefficient_cells": len(states) * 256,
            "covariance_coefficient_cells": len(states) * 784,
            "mean_coefficient_cells": len(states) * 64,
            "predictions": len(states) * 3,
        },
    }
    _native_json(result)
    return result
