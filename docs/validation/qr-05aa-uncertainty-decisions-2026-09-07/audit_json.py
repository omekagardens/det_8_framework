"""Independent stdlib JSON-only QR-05AA raw finite-decision audit.
...
def audit_capture(path=None):
    ...
    doc = json.loads(data)
    eq(canon(doc), data, "canonical AA envelope bytes")
    fields(doc, "schema_version runtime source_ledger prior_artifacts suite", "AA envelope")
    native_eq(doc["schema_version"], "det8-qr05aa-results-v1", "AA schema")
    ...
    native_eq(sorted(doc["source_ledger"]), sorted(names), "seven exact AA source filenames")
    inherited = []
    ...
    expected_priors = {**zdoc["prior_artifacts"], Z_PRIOR: Z_ID}
    native_eq(len(expected_priors), 31, "31-prior lineage size")
    native_eq(doc["prior_artifacts"], expected_priors, "exact AA inherited artifact lineage")
    ...
    for directory, prior in ((ZDIR, zdoc), (YDIR, ydoc), (XDIR, xdoc), (WDIR, wdoc)):
    ...
    zsuite, ysuite, xsuite, wsuite, suite = (
        zdoc["suite"],
        ydoc["suite"],
        xdoc["suite"],
        wdoc["suite"],
    ...
    expected_cases, expected_zcases, expected_ycases, expected_xcases = [], [], [], []
    for case, zcase, ycase, xcase, wcase in zip(
        suite["cases"],
        zsuite["cases"],
        ysuite["cases"],
        xsuite["cases"],
        ...
        problem = aa_projection(zanalysis)
        native_eq(problem, case["problem"], "entire AA risk table from literal raw-Z projection")
        analysis = expected_aa(problem)
        native_eq(
            analysis, case["analysis"], "entire AA independent pairwise-extrema decision wire"
        )
        controls_aa = aa_producer_controls(zanalysis, problem, analysis)
        native_eq(controls_aa, case["producer_controls"], "entire AA raw-law producer controls")
    ...
    native_eq(suite, expected_suite, "complete AA native canonical reporting closure")
    for current in (doc, zdoc, ydoc, xdoc):
        runtime_metadata(current["runtime"])
    for source, record in frozen.items():
        native_eq(ident(source), record, "after source/prior identity " + str(source))
    native_eq(ident(capture), before, "read-only AA artifact identity")
    ...
    return {
        ...
        "immutable_prior_artifacts": 31,
        "source_content_used_only_for_identity": True,
        "executor_runner_test_imports": False,
        "complete_native_canonical_X_Y_Z_and_AA_suites_checked": True,
