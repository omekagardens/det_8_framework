"""Independent ordered-deletion-word reference for QR-05S.

Raw features use directed chains, induced count-law moments, independent motif
recognizers and exact tensor interpolation carried locally from this route.
The local-only constructor has no observation or full-profile input: it counts
one fixed odd-then-even deletion word, divides by same-color factorials, then
checks all-target recurrences, rank normalization and local operator identities.
No prior artifact, executor, mutable core or source file is read at runtime.
This is a bounded deletion-law certificate, not a physical-time interpretation.
"""
...
def _parse(problem):
    ...
    _fields(problem, ("schema_version", "family", "model"), "invalid S problem fields")
    _require(
        type(problem["schema_version"]) is str
        and problem["schema_version"] == "det8-qr05s-problem-v1",
        "invalid S schema",
    )
    _require(
        type(problem["family"]) is str and problem["family"] == "qr05s_local_deletion",
        "invalid S family",
    )
    model = problem["model"]
...
def _partition(model, features):
    ...
    _require(len(classes) <= 512, "H class resource bound")
...
def _local_rows(model, features, partition):
    lookup = {
        _observation_key(s["frame_id"], s["kept"], s["past"]): s["state_id"]
        for s in model["states"]
    }
    ...
    for state, feature in zip(model["states"], features, strict=True):
        ...
        for vertex in sorted(set(state["kept"]) & set(frame["eligible"])):
            kept = [v for v in state["kept"] if v != vertex]
            key = _observation_key(state["frame_id"], kept, _induced(state, kept))
            _require(key in lookup, "single-deletion successor absent")
            target = partition["state_classes"][lookup[key]]
            color = 0 if vertex % 2 else 1
            rows[color][target] = rows[color].get(target, 0) + 1
            choices += 1
        result.append(
            {
                ...
                **{
                    name: [
                        ...
                        for target, count in sorted(rows[color].items())
                    ]
                    for color, name in enumerate(("odd", "even"))
                },
            }
        )
    _require(choices <= 26682, "fine local choice resource bound")
...
def _direct_profiles(model, features, partition):
    census = _subset_census(model, partition["state_classes"])
    colors = [features[g["members"][0]]["color_sizes"] for g in partition["classes"]]
    profiles = []
    for sid, counts in enumerate(census):
        O, E = features[sid]["color_sizes"]
        ranks = _zero_table()
        by_target = {}
        for (target, o, e), count in sorted(counts.items()):
            _require([o, e] == colors[target], "direct target color rank")
            _require(target not in by_target, "direct target has multiple color ranks")
            _integer(count, 1, min(9, _choose(O, o) * _choose(E, e)))
            by_target[target] = {
                "target_class": target,
            ...
            ranks[o][e] += count
        for o, e in product(range(4), repeat=2):
            _require(ranks[o][e] == _choose(O, o) * _choose(E, e), "direct rank normalization")
        profiles.append(
            {
                "state_id": sid,
                "parent_color_sizes": [O, E],
                "transitions": [by_target[target] for target in sorted(by_target)],
            }
        )
    return profiles
...
def analyze(problem):
    ...
    features, fine, _, _ = _features_and_fine(model)
    partition = _partition(model, features)
    local_rows, choices = _local_rows(model, features, partition)
    closure, local_classes, reconstruction = _closure(partition, local_rows)
    direct = _direct_profiles(model, features, partition)
    full_closed = True
    for group in partition["classes"]:
        reference = direct[group["members"][0]]["transitions"]
        for sid in group["members"][1:]:
            if direct[sid]["transitions"] != reference:
                full_closed = False
    _require(full_closed == closure["closed"], "local/full criterion disagreement")
    pushed = _push(fine, partition["state_classes"])
    expanded = _expand_profiles(direct)
    _require(_canonical(expanded) == _canonical(pushed), "direct profile/pushed power disagreement")
    rebuilt = None
    quotient = None
    if reconstruction is not None:
        rebuilt = []
        for sid, cid in enumerate(partition["state_classes"]):
            profile = reconstruction["class_profiles"][cid]
            rebuilt.append(
                {
                    "state_id": sid,
        ...
        _require(
            _canonical(rebuilt) == _canonical(direct),
            "local-only/raw-subset reconstruction mismatch",
        )
        quotient = [
            {
                "class_id": group["class_id"],
    ...
    profile_atoms = sum(len(row["transitions"]) for row in direct)
    result = {
        ...
        "comparison": {
            ...
            "reconstructed_profiles_sha256": None if rebuilt is None else _digest(rebuilt),
            "full_profile_closed": full_closed,
            "local_full_equivalent": full_closed == closure["closed"],
            "fine_kernel_sha256": _digest(fine),
            "pushed_rows_sha256": _digest(pushed),
            "quotient_rows_sha256": None if quotient is None else _digest(quotient),
            "profile_cells": 16 * profile_atoms if closure["closed"] else 0,
        ...
        "counts": {
            ...
            "local_atoms": sum(len(row[color]) for row in local_rows for color in ("odd", "even")),
            "class_local_atoms": sum(
                len(row[color]) for row in local_classes for color in ("odd", "even")
            ),
            "direct_profile_atoms": profile_atoms,
            "class_profile_atoms": 0
            if reconstruction is None
            else sum(len(row["transitions"]) for row in reconstruction["class_profiles"]),
        },
    }
    _native_json(result)
    _require(len(_canonical(result)) <= _MAX_WORK, "analysis output byte cap")
    return result
