...
separate states. The user requested regular scoped commits and pushes. This
inventory prevents accepted local work from being either forgotten or bundled
with unrelated changes to make an import succeed.
...
Independent source and mathematical review found no publication blocker.

Verification in the isolated candidate passed **203 focused pytest cases**
(47 kernel, 48 certificate, 108 identifiability) and **90 legacy research
checks** across nine groups, with no failures or errors. Only eight runner
groups differ from the published runner; the ninth kernel group is already
...
The identifiability interface certifies a supplied rational linear mean map
on unrestricted real parameters. It returns either a row-space reconstruction
...
`contracts.py`, `covariance.py`, `inference.py`, `provenance.py`, `replay.py`,
`validation_resources.py` and `workflow.py`. It also needs the modified
`relational_tomography.py` helpers `_chi_square_quantile`, `_finite_vector`
and `_mahalanobis_from_cholesky`; the published scheduler is sufficient.
No `det8.core` module was loaded or required in the captured demo closure.
The RET provenance inventory hashes 42 paths, while the captured run loads
18 local sources: 29 hashed paths are not loaded, and two applied helpers plus
three package initializers are loaded without inclusion in that identity.
That inventory is neither the execution closure nor a complete binding of
the applied demo. Publishing every hashed SDK path is not a substitute for
reviewing the actual prerequisites and provenance contract.

The five deferred claims paths are `det8/claims.py`,
`scripts/export_claim_registry.py`, `docs/CLAIM_REGISTRY.md`,
`det8/tests/test_claim_boundaries.py` and `det8/tests/test_claim_summary.py`.
The module's standard-library runtime does not make the package standalone:
it advertises unpublished core support and RET preview surfaces. Its checks
also need current event/legacy/validation boundaries and Track A/gravity
refusals absent from the published source. The generated reference has 35
distinct file targets: 23 are already identical remotely, four are modified
locally and eight are absent remotely at the audit base. Historical G1 source
evidence must not be inherited by later source changes. Preserving the legacy
claim JSON digests means preserving the local accepted baseline; it is not a
backward-compatibility claim about an already published claims API. These
claim checks do not directly require the whole RET implementation or G2 banks.

The measured pilot remains dependent on a selected instrument/dataset,
...
RI-26 now has a separate active QR reservation for the supplied-geometry
observer/signal model. Its code and results are not accepted or staged in
advance. The next coordinator checkpoint changes only root plan, progress,
this backlog and the data/follow-up note to record RI-25 publication and the
actual secondary dispatch. It preserves all twelve qubit-bundle files and
the other 75 held predecessors.


**RI-26 accepted implementation — complete publication candidate**

The supplied-geometry [observer/signal experiment](../experiments/observer_signal_v1/EXPERIMENT.md)
is independently accepted at the eleven source identities recorded in
[progress](REVIEW_PROGRESS.md). Root's copied candidate passed all 51 tests
in normal and optimized Python; both direct worked runs produced 52 identical
exports. A separate JSON-only audit passed 331 assertions. Final mathematical,
source/data-flow and documentation reviews found no remaining blocker.

The QR reservation is released. Stage exactly eleven bundle files, the three
QR handoff/map summaries and four coordinator records after the preservation
and link checks. All ten Python sources match the tested candidate; the note
has only the reviewed final verification metadata delta. The publication
also records two additional bounded measured-data leads and corrects stale
RI-25 publication metadata. No external scientific observations are imported.

The experiment is complete as a conditional supplied-geometry model. Measured
...
**RI-27 accepted reproduction handoff — five-path candidate**
...
Independent reviews passed. Actual replay passed 61 qubit and 51 observer
tests per mode, matched twelve and 52 exports, and retained identical
23-file before/after integrity reports and unchanged extracted source.
The candidate changes only these two companions and plan/progress/backlog.
No new source prerequisite, registered witness, data acquisition or QR
successor is introduced. Existing measured/application dependencies remain.
...
update preserves every accepted experiment source. No successor is assigned.


**RI-31 native geometry, gravity and measurement planning**

The [new plan](NATIVE_GEOMETRY_GRAVITY_MEASUREMENT_PLAN.md) answers the user's
request with simulations and public data first. Its first proposed QR packet
is a fully specified native-input joint law with an all-size/domain theorem
or family-scoped obstruction. Parallel measurement work begins with an
observation channel, analytic nonidentifiability examples, synthetic fixtures
and one qualified conventional public-data baseline. Native manifold
existence, local measure, matter/source response, gravity limits and a
quantitative physical comparison retain their distinct premises.

Independent geometry, gravity and measurement reviews and QR scope review
passed after three precise premise clarifications. Publish only the new
programme and plan/progress/backlog. It has no executable dependency on the
unpublished T8/RET baseline;
those local sources are reference evidence only. Existing models, maps,
...
**RI-32 accepted family-scoped rejection**

The [single dossier](../track_b/native_joint_growth_v1/DOSSIER.md) is accepted
after root full-document review and an independent mathematical/scope review.
The exact positive all-zero sector gives unequal full-payload birth maps
D/24 and D/32 for every admitted coupling. Positive coupling also violates
the specifically declared strict precursor-record permission. This rules
out only that family, not all native laws, and supplies no measured evidence.

Publish exactly the new dossier plus three coordinator records. All five
document references are published background; the untracked T8 source is
...
**RI-34 accepted normalization theorem and prefix classification**

The [proof note](../track_b/native_joint_growth_constraints_v1/NORMAL_FORM.md)
passed complete root review and two independent coordinator reviews. Boolean
normalization/positivity, compensating marginal responses, every stated
early full-payload diamond, weak endpoints and null-mass sectors remain
explicitly separated. This is conditional mathematics and a finite-prefix
...
**RI-39 analytic checkpoint and RI-41 continuation**

Publish only `docs/track_b/native_growth_history_height_v1/HISTORY_HEIGHT.md`
and the three coordinator records. Root and an independent coordinator reviewer
read the complete proof and independently checked its primary measure theorem.
The cylinder measure and conditional height screen are accepted; no numerical
execution was needed. One administrative opening phrase is the only change
from the handed-off note. The sole local prerequisite is published RI-38.

RI-41 is assigned the fixed-prefix normalization decision after that review.
Its new sources and RI-40's active calibration bundle are excluded. The
...
execution, independent arithmetic/custody adjudications and all scope boundaries.
The native result is the fixed positive-amplitude obstruction, not a general
harmonic-extension impossibility. Measurement qualification is fabricated;
actual saved-input application and empirical adequacy remain distinct. Required
external launchers, runtime, receipts and complete qualification artifacts stay
at their pinned durable evidence paths. Historical source-only snapshots remain
unchanged. RI-99 proof/design and RI-100 concrete application preparation are
...
reviews pass; all source premises remain pinned. The local rank-two positive
slice is not a global extension; full signed/positive feasibility remains open.
RI-101 proof/design is assigned externally after adjudication. RI-100's delivered
external caller, pending source/admission work and all actual execution outputs
are excluded from this checkpoint. Root preserves the exact source dependencies,
...
pass at the unchanged worker identity. The full signed one-layer question is
settled; the prescribed-amplitude positive lift remains a separate RI-102 QR
assignment. No all-size/physical claim or amplitude change is published.

Actual RI-100 normal and optimized applications pass all eleven gates and have
identical complete output bytes; root and independent execution-custody reviews
pass. Their outputs remain external pending the separately authored arithmetic
audit. Its complete source/contract is accepted and a minimal execution adapter
is assigned externally. This checkpoint excludes scientific result publication,
active adapter sources and all unrelated working-tree changes. The latest
...
The [RI-104 observed-data design](../experiments/gwosc_off_event_context_benchmark_v1/DESIGN.md)
...
is still excluded pending actual independent saved arithmetic. RI109 final caller
binding and RI110 observed caller preparation remain external active source work.
...
exact v1 reversal and root applicability decision are recorded in the progress
