...
## 11. Historical RI-20 assignment — consolidated endpoint and upstream premises
...
## 12. Accepted composition-premise audit and probe criterion — RI-20

The [RI-20 note](QR_COMPOSITION_PREMISE_PLAN.md) closes its assigned proof
and source-audit obligations. For fixed normalized PSD ancillary probes B and
Boolean event incidence R, T=(RBR^T)^T is PSD and the observed mathematical
weight is Tr(DT). Including every base indicator, the closed conic test hull
K forces every normalized Hermitian test-positive candidate D to be PSD iff
K=PSD_n. For a proper K, cone separation followed by a sufficiently small
positive identity shift gives a legally normalized, weak-positive non-PSD
counterexample with strictly positive values on every nonzero test.

The result concerns unrestricted Hermitian candidates and exact fixed linear
inequalities. Relative candidate restrictions need a relative criterion;
noisy estimates need an error contract. A finite fixed menu is inadequate for
n>=2, but nonlinear candidate self-powers and infinite families generated by
finite alphabets have different quantifiers. Finitely many informative exact
scalar values can also support reconstruction followed by a nonlinear PSD
check; RI-20 excludes inference from their nonnegativity alone, not arbitrary
inference from their values. Such an observation contract still needs its
actual resource catalogue and calibration.

Three independent coordinator reviews accepted the proof, complex transpose,
normalization, countermodels, literature hypotheses and repository ledger.
The known tensor-closed-class containment theorem does not supply DET's
missing pre-PSD admission, arbitrary finite mutual composition or interference
resource. Those remain additional candidate premises unless independently
justified. The original F3 forcing question therefore remains open despite
completion of this bounded audit.

The selected next proof target is a constructive principal-minor event using
at most n identical candidate copies and one specified signed seed. This is a
