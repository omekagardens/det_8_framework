...
0.88–1.07 at `n = 250`), so this is a bounded discrimination test, not a theorem. 10 tests
pass (chain, bipartite and layer-cake known answers). See
...
established**. The open question is now a **stronger adversary** (a growth law engineered to
match the higher-order invariant at all interval sizes — a covariance/self-similar-ensemble
match) with a provenance-independent bound. With DD the scale-free
...
| QR-05DL: a higher-order manifoldlike invariant — the interval self-similarity (complete, executable; discriminator found, no no-go) | Interval self-similarity `S = ⟨f(I(x,y))⟩/f_global` over intervals of size `[3, 30]` at `n = 250`: sprinkles `d = 2, 3, 4` give `S = 1.07, 0.88, 0.92` and reproduce the global Myrheim–Meyer dimension to `|Δd_MM| ≤ 0.16`; DK's counterexamples fail — the bipartite order has **no intervals** (density 0, `S` undefined), the layer-cake's intervals are whole layers (outside the window), and transitive percolation at `p = 0.02` has `S = 6.77`, `|Δd_MM| = 2.46`; two routes agree, 10 tests pass (chain, bipartite and layer-cake known answers) | Closes DK's gap — a higher-order invariant does separate the geometry-free counterexamples from manifoldlike sets — but manifoldlikeness is **multi-faceted** (no single invariant — 2-point, link, or interval self-similarity — suffices) and **no no-go is established**; `S` is finite-size biased (bounded discrimination test, not a theorem); the next target is a stronger adversary (a law matching the invariant at all interval sizes); no metric, continuum, curvature or gravity claim |
| Next: a stronger adversary for the higher-order invariant (a law matching the interval self-similarity), an unconditional uniform LPP (longest-chain) concentration bound, density/commensurability singularities, or the empirical bridge (Branch B; OPEN — physical evidence required) | A law matching `S` (and the covariance/self-similar ensemble) at all interval sizes; a proof of the uniform LPP fluctuation bound; the density/commensurability condition (where V∝τ^d can fail); ensemble convergence — or the BX apparatus | Branch A: remaining gaps are the unconditional LPP-concentration proof and a stronger higher-order adversary (DL closes the DK gap, establishes no no-go); Branch B remains open until physical evidence is supplied; do not promote; curvature and the BD operator stay parked; κ-gravity retired, gravity standard GR |
