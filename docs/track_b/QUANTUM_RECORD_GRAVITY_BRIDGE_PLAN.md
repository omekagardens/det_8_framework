...
| Next: a covariance-matched adversary (a law matching the whole interval-size profile), an unconditional uniform LPP (longest-chain) concentration bound, density/commensurability singularities, or the empirical bridge (Branch B; OPEN — physical evidence required) | A law matching the whole interval-abundance distribution (including the interval-in-interval structure); a proof of the uniform LPP fluctuation bound; the density/commensurability condition (where V∝τ^d can fail); ensemble convergence — or the BX apparatus | Branch A: remaining gaps are the unconditional LPP-concentration proof and a covariance-matched adversary (DL found the invariant, DM showed the `f`-matched law fails and that the mean alone is too weak; no no-go); Branch B remains open until physical evidence is supplied; do not promote; curvature and the BD operator stay parked; κ-gravity retired, gravity standard GR |
